...
fn virtio_net_pings_host_tap() {
    ...
    assert!(
        output.contains("NET_OK"),
        "network marker missing:\n{output}"
    );
...
fn virtio_net_bulk_throughput() {
    ...
    let cmdline = common::cmdline_with(
        "net_throughput",
        &[
    ...
    assert!(
        output.contains("THROUGHPUT_OK"),
        "throughput marker missing:\n{output}"
    );
    common::assert_no_panic(&output);
...
/// guest's connection to 10.0.2.2:<port> lands on the host sink. `extra_markers`
/// are additional guest serial markers that must appear (e.g. a DHCP lease).
fn user_net_transfer(mode: &str, extra_markers: &[&str]) {
    ...
    assert!(
        output.contains("USERNET_OK"),
        "user-net marker missing:\n{output}"
    );
    for marker in extra_markers {
        assert!(
            output.contains(marker),
            "expected user-net marker {marker:?} missing:\n{output}"
        );
    }
    common::assert_no_panic(&output);
...
/// byte count. Bounded by `deadline` so a stalled or absent guest can't hang the
/// test; returns whatever was read so far when the deadline elapses.
