...
pub fn vmm() -> Command {
    ...
    if matches!(std::env::var("VMON_JAIL"), Ok(ref v) if v == "1") {
        ...
        let jail_root = fs::canonicalize(&jail_root)
            .unwrap_or_else(|e| panic!("canonicalizing jail root: {e}"));
        cmd.arg("--id").arg(&id);
...
const INITRAMFS_ASSETS: &[&str] = &[
    "busybox-initramfs-x86_64.cpio.gz",
    "busybox-initramfs.cpio.gz",
];
#[cfg(target_arch = "aarch64")]
const INITRAMFS_ASSETS: &[&str] = &[
    "busybox-initramfs-aarch64.cpio.gz",
    "busybox-initramfs.cpio.gz",
];
...
pub fn kernel() -> PathBuf {
    if let Some(path) = std::env::var_os("VMON_KERNEL") {
        ...
        assert!(
            path.is_file(),
            "VMON_KERNEL points at missing file {}",
            path.display()
        );
...
fn first_existing_asset(names: &[&str], label: &str) -> PathBuf {
    for name in names {
        ...
        if path.is_file() {
            return fs::canonicalize(&path).unwrap_or_else(|e| {
                panic!("canonicalizing {label} asset {}: {e}", path.display())
            });
        }
    }
    panic!(
        "missing {label} in {TEST_ASSETS} (looked for {names:?}); run demo/fetch-test-assets.sh before e2e tests"
    );
}
    ...
    pub fn wait_for(&mut self, marker: &str, timeout: Duration) -> String {
        ...
        loop {
            ...
            if now >= deadline {
                ...
                self.kill();
                panic!(
                    "timed out after {timeout:?} waiting for marker {marker:?}; output:\n{output}"
                );
            }
