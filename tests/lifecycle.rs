...
fn json_control_ping_info_errors_pause_snapshot_resume_quit() {
    ...
    assert_eq!(
        denied.get("code").and_then(|v| v.as_str()),
        Some("path_denied")
    );
    let running = control.request_error("snapshot", json!({"name": "running"}));
    assert_eq!(
        running.get("code").and_then(|v| v.as_str()),
        Some("not_paused")
    );
    control.ping();
    ...
    assert!(
        status.success(),
        "vmon exited with {status}; output:\n{output}"
    );
...
fn two_vcpu_busy_guest_pause_resume_survives() {
    ...
    assert!(
        boot.contains("CPUS=2"),
        "SMP guest did not expose 2 CPUs:\n{boot}"
    );
    ...
    assert!(
        vm.has_exited().is_none(),
        "busy SMP guest exited after pause/resume:\n{}",
        vm.output()
    );
    ...
    assert!(
        status.success(),
        "busy SMP guest exited with {status}; output:\n{output}"
    );
    common::assert_no_panic(&output);
}
