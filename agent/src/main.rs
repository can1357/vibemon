...
mod linux_agent {
    ...
    impl Agent {
        ...
        fn start_exec(&self, id: u32, request: &Value) {
            ...
            let (stdin, tty_master, joins) = if let Some((master_fd, slave_fd)) = pty_fds {
                ...
                let join =
                    spawn_stream_thread(self.writer.clone(), id, proto::FRAME_STDOUT, reader);
        ...
        fn mount(&self, id: u32, request: &Value) {
            ...
            if rc != 0 {
                self.send_error(
                    id,
                    format!("mount {tag} -> {path}: {}", io::Error::last_os_error()),
                );
                return;
            }
            self.send_resp(id, json!({"ok": true}));
    ...
    fn parse_netlink_ack(buf: &[u8]) -> io::Result<()> {
        ...
        let err = unsafe {
            std::ptr::read_unaligned(buf[header_len..].as_ptr().cast::<libc::nlmsgerr>())
        };
        if err.error == 0 {
            Ok(())
        } else {
