...
//! paused-then-snapshotted VM through [`Vmm::restore`] without booting a kernel.
//! Once running, an optional Unix control socket drives pause/resume/snapshot.
...
use crate::hv::Gic;
use crate::hv::{Exit, Vcpu, Vm};
use crate::os::{EFD_NONBLOCK, EventFd};

use crate::arch;
use crate::config::{BootMode, Config, MAX_COUNT, Transport};
use crate::control::{self, ControlCmd, ControlKind, PauseGate, RunState, VcpuCmd};
use crate::devices::Bus;
use crate::devices::serial::SerialDevice;
use crate::layout::{IRQ_BASE, MMIO_DEVICE_SIZE, MMIO_MEM_START, SERIAL_IRQ};
#[cfg(target_arch = "x86_64")]
use crate::layout::{
    PCI_CONFIG_IO_BASE, PCI_CONFIG_IO_SIZE, PCI_VIRTIO_BAR_SIZE, PCI_VIRTIO_MMIO_SIZE,
    PCI_VIRTIO_MMIO_START,
};
use crate::metrics::VmExit;
...
use crate::virtio::console::OUTPUT_CAP;
use crate::virtio::fs::Fs;
use crate::virtio::mmio::{MmioState, MmioTransport};
...
use crate::virtio::{Interrupt, VirtioDevice, WorkerControl, run_worker};
...
/// VMM-side handle to a wired virtio device: lets the lifecycle pause/resume the
/// worker and read transport/queue/interrupt state for a snapshot.
struct DeviceHandle {
    kind: DeviceKind,
    mmio_base: u64,
    gsi: u32,
...
pub struct Vmm {
    ...
    /// Prepared Linux vCPUs (configured/restored), drained into threads by `start`.
    #[cfg(target_os = "linux")]
    vcpus: Vec<Vcpu>,
    /// Plain startup payload used to create/configure HVF vCPUs on their owning threads.
    #[cfg(target_os = "macos")]
    ...
    /// Host end of the virtio-console agent channel (input buffer + wake eventfd),
    /// present when `--console-agent` is set. Used to push bytes into the guest.
...
#[allow(dead_code, reason = "MachineInfo will be used more extensively in forthcoming VMM features")]
struct MachineInfo {
    cpus: u8,
    mem_mib: usize,
    state: RunState,
...
pub fn run_inner_with_prebound(config: Config, mut prebound: PreboundSockets) -> Result<()> {
    ...
    let agent_socket = match prebound.agent_socket.take() {
        ...
        None => match bind_agent_socket(agent_sock, jail, prebound.launch_uid) {
            Ok(socket) => socket,
            Err(e) => {
                cleanup_control_server(&control_server);
                return Err(e);
            }
        },
    };
    ...
    let pager_setup = match config.mem_target_mib {
        Some(target_mib) => {
            ...
            match setup {
                Ok(setup) => Some(setup),
                Err(e) => {
                    cleanup_agent_socket(&agent_socket);
                    cleanup_control_server(&control_server);
                    return Err(e);
                }
            }
    ...
    if let Err(e) = vmm.start() {
        ...
        return match shutdown {
            ...
            Err(shutdown_err) => Err(err(format!(
                "starting VM failed: {e}; shutdown failed: {shutdown_err}"
            ))),
        };
    }
    ...
    // Hand the entrypoint to the in-guest agent over the console (warm-boot exec).
    if let Some(cmd) = &agent_exec {
        if let Err(e) = vmm.agent_send(cmd) {
            ...
            return match shutdown {
                Ok(()) => Err(e),
                Err(shutdown_err) => Err(err(format!(
                    "sending agent exec failed: {e}; shutdown failed: {shutdown_err}"
                ))),
            };
        }
...
fn bind_agent_socket(
    ...
    if let Err(e) = fs::set_permissions(&sock_path, fs::Permissions::from_mode(0o600)) {
        ...
        return Err(err(format!(
            "chmoding agent socket {} to 600 failed: {e}",
            sock_path.display()
        )));
    }
    listener.set_nonblocking(true).map_err(|e| {
        cleanup_agent_socket_path(&sock_path);
        err(format!(
            "setting agent socket {} nonblocking failed: {e}",
            sock_path.display()
        ))
    })?;
    Ok(Some(AgentSocket {
    ...
    match fs::symlink_metadata(parent) {
        ...
        Err(e) if e.kind() == io::ErrorKind::NotFound => {
            fs::create_dir_all(parent).map_err(|e| {
                err(format!(
                    "creating agent socket parent {} failed: {e}",
                    parent.display()
                ))
            })?;
            fs::set_permissions(parent, fs::Permissions::from_mode(0o700)).map_err(|e| {
                err(format!(
                    "chmoding agent socket parent {} to 700 failed: {e}",
                    parent.display()
                ))
            })?;
        }
        Err(e) => {
            return Err(err(format!(
                "checking agent socket parent {} failed: {e}",
                parent.display()
            )));
        }
    }
...
fn remove_stale_agent_socket(sock_path: &Path) -> Result<()> {
    let meta = match fs::symlink_metadata(sock_path) {
        ...
        Err(e) => {
            return Err(err(format!(
                "checking agent socket {} failed: {e}",
                sock_path.display()
            )));
        }
    };
    ...
    match UnixStream::connect(sock_path) {
            ...
            ) =>
        {
            fs::remove_file(sock_path).map_err(|e| {
                err(format!(
                    "removing stale agent socket {} failed: {e}",
                    sock_path.display()
                ))
            })
        }
        Err(e) => Err(err(format!(
            "probing existing agent socket {} failed: {e}",
            sock_path.display()
        ))),
    }
}
    ...
    loop {
        ...
        if let Some(stream) = client.as_mut() {
            if client_revents & libc::POLLIN != 0 {
                match read_agent_socket_to_console(
                    stream,
                    &console_input,
                    &input_evt,
                    &mut input_buf,
        ...
        if let Some(stream) = client.as_mut()
            ...
            && drain_guest_output_to_socket(stream, &output, &drain_resume_evt).is_err()
        {
            client = None;
            crate::metrics::record_agent_bridge_disconnect();
            discard_guest_output(&output, &drain_resume_evt);
...
fn verify_agent_peer_credentials(stream: &UnixStream, launch_uid: u32) -> io::Result<()> {
    // SAFETY: `cred` is plain old data and will be fully initialized by a
    // successful `getsockopt` before its fields are read.
    ...
    Err(io::Error::new(
        ...
        format!(
            "agent socket peer uid {} is not root or launch uid {launch_uid}",
            cred.uid
        ),
    ))
}
...
/// Open (create + truncate) `status.json` beside the control socket so it can be
/// written at exit through a held fd. Opened *before* the privilege drop and
/// Landlock: DAC is checked at open (so the dropped sandbox uid can still write)
/// and Landlock path rules do not restrict writes to already-open fds.
...
fn parse_snapshot_boot_mode(value: &str) -> Result<BootMode> {
    match value {
        ...
        other => Err(err(format!(
            "snapshot boot_mode {other} is not supported"
        ))),
    }
}
...
/// each forking one clone with a unique tap, MAC, and `CoW` disk overlay. `--count
/// 1` is forced on the children so they take the single-fork path (no recursion).
fn spawn_fork_children(dir: &Path, config: &Config) -> Result<()> {
    ...
    for i in 0..count {
        ...
        info!(
            "fork child {i}: tap=tap{i} mac=02:00:00:00:00:{:02x}",
            i + 1
        );
        let child = cmd
            .spawn()
...
impl Vmm {
    ...
    fn build(config: Config) -> Result<Self> {
        ...
        let serial = setup_serial(&vm, &mut pio_bus, &mut mmio_bus, None)?;
        ...
        if let (Some(tag), Some(dir)) = (&config.fs_tag, &config.fs_dir) {
            let shared_dir = std::fs::canonicalize(dir)
                .map_err(|e| err(format!("virtio-fs shared dir {dir:?}: {e}")))?;
        ...
        for m in &config.volumes {
            let shared_dir = std::fs::canonicalize(&m.dir)
                .map_err(|e| err(format!("virtio-fs volume dir {:?}: {e}", m.dir)))?;
            let fs_dev = Arc::new(Mutex::new(Fs::new(
                m.tag.clone(),
                shared_dir.clone(),
                m.read_only,
            )?));
        ...
        #[cfg(target_os = "macos")]
        let macos_startup = MacosStartup::Boot {
            entry: loaded.entry,
            initrd,
            virtio_infos,
        };
    ...
    /// Fork (`CoW`) from a template snapshot: child RAM is a `MAP_PRIVATE` mapping
    /// of the manifest-selected memory file, so clean pages are shared via the
    /// host page cache across children while writes fault to private copies.
    ...
    fn reconstruct(vm: Arc<Vm>, snap: Snapshot, config: &Config) -> Result<Self> {
        ...
        let firmware_memory = match boot_mode {
            ...
            BootMode::Uefi => {
                ...
                Some(arch::boot::map_uefi_firmware(
                    Path::new(path),
                    vm.memory(),
                    &vm.fd,
                )?)
        ...
        #[cfg(target_os = "macos")]
        let macos_startup = MacosStartup::Restore {
        ...
        let mut pio_bus = Bus::new();
        let mut mmio_bus = Bus::new();
        let serial = setup_serial(&vm, &mut pio_bus, &mut mmio_bus, Some(&snap.serial))?;
        ...
        for ds in &snap.devices {
            ...
            let device: Arc<Mutex<dyn VirtioDevice>> = match &backend {
                BackendHint::Block { path, read_only } => {
                    ...
                    let block = if let Some(base) = &config.disk_overlay_of {
                        let overlay =
                            crate::virtio::block::create_cow_overlay(base, Path::new(path))?;
    ...
    /// Spawn the device-worker and vCPU threads and put the host console in raw mode.
    ...
    fn start_macos_vcpus(&mut self, xsave_size: usize) -> Result<()> {
        ...
        match &self.macos_startup {
            MacosStartup::Boot {
                ...
                let fdt_addr = configure_aarch64_fdt(
                    &self.vm,
                    &self.cmdline,
                    *initrd,
                    virtio_infos,
    ...
    /// Block until the guest exits (no control socket) or process control commands.
    fn serve_and_wait(
        mut self,
        control_server: Option<control::ControlServer>,
        ...
        if let Some(server) = control_server {
            ...
            exit = loop {
                ...
                let wait = match self.deadline {
                    Some(deadline) => Duration::from_millis(200)
        ...
        match kind {
            ...
            ControlKind::Snapshot { name, base } => {
                ...
                if self.gate.state() != RunState::Paused {
                    return Err(control::ApiError::not_paused(
                        "snapshot requires a paused VM",
                    ));
                }
                self.snapshot(&dir, base.as_deref())
                    .map(|()| json!({}))
                    .map_err(|e| control::ApiError::internal(e.to_string()))
            }
            ControlKind::Quit => {
    ...
    fn recv_vcpu_state(
        ...
        loop {
            ...
            if Instant::now() >= deadline {
                return Err(err(format!(
                    "snapshot vcpu save timed out after {SNAPSHOT_VCPU_STATE_TIMEOUT:?} for vCPU {vcpu_id}"
                )));
            }
            let wait = min_duration(
                PAUSE_WAIT_POLL_INTERVAL,
            ...
            match rx.recv_timeout(wait) {
                ...
                Ok(Err(e)) => {
                    return Err(err(format!(
                        "snapshot vcpu save failed for vCPU {vcpu_id}: {e}"
                    )));
                }
                Err(RecvTimeoutError::Timeout) => {}
                Err(RecvTimeoutError::Disconnected) => {
    ...
    /// Push a line to the in-guest agent over the virtio-console channel (the
    /// warm-boot entrypoint hand-off). Fails fast if no `--console-agent` device
    /// is attached, so a misconfigured warm boot never silently hangs.
    fn agent_send(&self, line: &str) -> Result<()> {
        let (buf, evt) = self
            .console_input
            .as_ref()
    ...
    /// If `base` is `Some`, store only the pages that differ from that sibling snapshot.
    fn snapshot(&mut self, dir: &Path, base: Option<&str>) -> Result<()> {
        ...
        for (vcpu_id, tx) in self.vcpu_cmd_txs.iter().enumerate() {
            ...
            tx.send(VcpuCmd::SaveState(rtx)).map_err(|_| {
                err(format!(
                    "snapshot vcpu save command failed for vCPU {vcpu_id}: vcpu thread gone"
                ))
            })?;
        ...
        let machine = arch::state::MachineState {
            gic: self.gic.save_state(u64::from(self.cpus))?,
        };
        #[cfg(all(target_os = "macos", target_arch = "aarch64"))]
        let machine = arch::state::MachineState {
            gic: self
        ...
        snapshot::write_snapshot(dir, base, self.vm.memory(), move |mem_regions, delta| {
            Snapshot {
...
fn wait_eventfd_read(evt: &EventFd, timeout: Duration) -> Result<bool> {
    ...
    if error_events != 0 {
        return Err(err(format!(
            "eventfd poll returned error revents=0x{:x}",
            pfd.revents
        )));
    }
...
fn fresh_net_backend(config: &Config) -> Option<BackendHint> {
    if config.user_net {
        ...
        config.tap.as_ref().map(|tap| BackendHint::Net {
            tap: tap.clone(),
            mac: config.mac,
        })
    }
...
/// can point at a different disk/tap/fs share); otherwise the snapshot hint is used.
fn resolve_backend(ds: &DeviceState, config: &Config) -> BackendHint {
    match &ds.backend {
        BackendHint::Block { path, read_only } => BackendHint::Block {
            path: config
                .rootfs
                .as_ref().map_or_else(|| path.clone(), |p| p.to_string_lossy().into_owned()),
            read_only: *read_only,
        },
        BackendHint::Net { tap, mac } => {
            ...
            if config.user_net {
                ...
                BackendHint::Net {
                    tap: config.tap.clone().unwrap_or_else(|| tap.clone()),
                    mac,
                }
            }
        }
        ...
        BackendHint::Fs {
            ...
            let (shared_dir, read_only) =
                if let Some(m) = config.volumes.iter().find(|m| &m.tag == tag) {
                    (
                        std::fs::canonicalize(&m.dir).map_or_else(|_| m.dir.to_string_lossy().into_owned(), |p| p.to_string_lossy().into_owned()),
                        m.read_only,
                    )
                    ...
                    (
                        config
                            .fs_dir
                            .as_ref().map_or_else(|| shared_dir.clone(), |p| p.to_string_lossy().into_owned()),
                        *read_only,
                    )
                } else {
...
fn setup_serial(
    ...
    #[cfg(target_arch = "x86_64")]
    _pio_bus.register(0x3f8, 8, serial.clone());
    #[cfg(target_arch = "aarch64")]
    _mmio_bus.register(
...
#[allow(clippy::too_many_arguments, reason = "Complexity is required due to the number of parameters needed to wire a device")]
fn wire_device(
    vm: &Vm,
    mmio_bus: &mut Bus,
    device: Arc<Mutex<dyn VirtioDevice>>,
...
fn wire_pci_device(
    ...
    let transport = match saved {
        ...
        Some(state) => {
            let pci = state.transport_pci.as_ref().ok_or_else(|| {
                err("snapshot PCI device is missing serialized PCI transport state")
            })?;
...
fn load_initrd(
    ...
    match &config.initrd {
        Some(path) => Ok(Some(arch::boot::load_initrd(path, vm.memory(), loaded)?)),
        None => Ok(None),
    }
}
...
fn configure_aarch64_fdt(
    ...
    use crate::arch::fdt::FdtDevice;
    use crate::layout::{MMIO_DEVICE_SIZE, SERIAL_IRQ, SERIAL_MMIO_BASE, SERIAL_MMIO_SIZE};

    let serial = FdtDevice {
        addr: SERIAL_MMIO_BASE,
        len: SERIAL_MMIO_SIZE,
        gsi: SERIAL_IRQ,
    };
    let virtio: Vec<FdtDevice> = virtio_infos
        .iter()
        .map(|&(addr, gsi)| FdtDevice {
            addr,
...
fn complete_hvf_mmio(
    ...
    if let Err(e) = vcpu.complete_mmio() {
        fatal(
            ...
            VmmExit::Killed,
            id,
            &format!("completing HVF MMIO failed: {e}"),
        );
        return false;
    }
    true
