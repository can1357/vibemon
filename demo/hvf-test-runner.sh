...
test_bin=$1
# The test binary lives at <target>/<profile>/deps/<name>-<hash>; the vmon
# binary it spawns (CARGO_BIN_EXE_vmon) is one directory up.
vmon="$(dirname "$test_bin")/../vmon"
entitlements="${VMON_ENTITLEMENTS:-hvf.entitlements}"

if [ -x "$vmon" ]; then
    codesign --sign - --entitlements "$entitlements" --force "$vmon" >/dev/null 2>&1 \
        || echo "hvf-test-runner: warning: failed to codesign $vmon" >&2
fi
