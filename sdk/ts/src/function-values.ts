...
export interface DecodeValueOptions { artifactStore?: ValueArtifactStore; }
...
export async function encodeValue(
  ...
  checkPortable(value, "$", new Set<object>());
  const serializer = options.serializer ?? "json";
  if (serializer !== "json" && serializer !== "cbor") {
    if (serializer === "cloudpickle") throw new Error("cloudpickle values are Python-only and cannot be encoded by TypeScript");
    throw new Error(`unsupported value serializer ${String(serializer)}`);
  }
  const compression = options.compression ?? "none";
  if (compression !== "none" && compression !== "gzip") throw new Error(`unsupported value compression ${String(compression)}`);
  const raw = serializer === "json" ? textEncoder.encode(JSON.stringify(value)) : encodeCbor(value);
  ...
  if (!Number.isSafeInteger(inlineLimit) || inlineLimit < 0) throw new RangeError("inlineLimit must be a non-negative safe integer");
  let storage: ValueEnvelope["storage"];
  if (stored.byteLength > inlineLimit) {
    if (!options.artifactStore) throw new Error("artifactStore is required for values exceeding inlineLimit");
    storage = {
      case: "artifact",
  ...
  return {
    ...
    typeNamePresence: options.typeName === undefined
      ? { case: undefined }
...
export async function decodeValue(
  ...
  if (envelope.schemaVersion !== 1) throw new Error(`unsupported value envelope schema ${envelope.schemaVersion}`);
  if (envelope.serializer === ValueSerializer.CLOUDPICKLE) throw new Error("cloudpickle values are Python-only and cannot be decoded by TypeScript");
  if (envelope.serializer !== ValueSerializer.JSON && envelope.serializer !== ValueSerializer.CBOR) throw new Error("unsupported value serializer");
  let stored: Uint8Array;
  if (envelope.storage.case === "inlineData") stored = envelope.storage.value;
  else if (envelope.storage.case === "artifact") {
    if (!options.artifactStore) throw new Error("artifactStore is required to decode an artifact-backed value");
  ...
  else if (envelope.compression === ValueCompression.GZIP) raw = await transformBytes(stored, "gzip", false);
  else throw new Error("unsupported value compression");
  if (BigInt(raw.byteLength) !== envelope.uncompressedSizeBytes) throw new Error("value envelope size mismatch");
  if (!envelope.checksum || envelope.checksum.algorithm !== DigestAlgorithm.SHA256) throw new Error("value envelope requires a SHA-256 checksum");
  if (!equalBytes(await sha256(raw), envelope.checksum.value)) throw new Error("value envelope checksum mismatch");
  const decoded: unknown = envelope.serializer === ValueSerializer.JSON
    ? JSON.parse(textDecoder.decode(raw))
    : decodeCbor(raw);
  checkPortable(decoded, "$", new Set<object>());
  return decoded;
}

function checkPortable(value: unknown, path: string, active: Set<object>): asserts value is PortableValue {
  ...
  if (Array.isArray(value)) {
    for (let index = 0; index < value.length; index += 1) checkPortable(value[index], `${path}[${index}]`, active);
  } else {
    const prototype = Object.getPrototypeOf(value);
    if (prototype !== Object.prototype && prototype !== null) throw new TypeError(`${path} must be a plain object`);
    for (const key in value) checkPortable(Reflect.get(value, key), `${path}.${key}`, active);
  }
  active.delete(value);
}
...
async function transformBytes(data: Uint8Array, format: "gzip", compress: boolean): Promise<Uint8Array> {
...
function writeCbor(value: PortableValue, output: number[]): void {
  if (value === null) { output.push(0xf6); return; }
  if (value === false) { output.push(0xf4); return; }
  if (value === true) { output.push(0xf5); return; }
  if (typeof value === "number") {
  ...
  for (const [key, item] of entries) { writeCbor(key, output); writeCbor(item, output); }
}
function writeHead(major: number, value: number, output: number[]): void {
  if (value < 24) { output.push((major << 5) | value); return; }
  if (value <= 0xff) { output.push((major << 5) | 24, value); return; }
  if (value <= 0xffff) { output.push((major << 5) | 25, value >>> 8, value & 0xff); return; }
  if (value <= 0xffffffff) {
    output.push((major << 5) | 26, (value >>> 24) & 0xff, (value >>> 16) & 0xff, (value >>> 8) & 0xff, value & 0xff);
    return;
  }
  const high = Math.floor(value / 0x1_0000_0000);
  const low = value - high * 0x1_0000_0000;
  output.push((major << 5) | 27, (high >>> 24) & 0xff, (high >>> 16) & 0xff, (high >>> 8) & 0xff, high & 0xff, (low >>> 24) & 0xff, (low >>> 16) & 0xff, (low >>> 8) & 0xff, low & 0xff);
}
...
class CborReader {
  ...
  done(): boolean { return this.#offset === this.data.byteLength; }
  value(): PortableValue {
  ...
  length(additional: number): number {
    ...
    if (additional === 26) return this.byte() * 0x1_000000 + this.byte() * 0x10000 + this.byte() * 0x100 + this.byte();
    if (additional === 27) {
      const high = this.byte() * 0x1_000000 + this.byte() * 0x10000 + this.byte() * 0x100 + this.byte();
      const low = this.byte() * 0x1_000000 + this.byte() * 0x10000 + this.byte() * 0x100 + this.byte();
      const value = high * 0x1_0000_0000 + low;
      if (!Number.isSafeInteger(value)) throw new Error("CBOR integer exceeds JavaScript safe integer range");
      return value;
    }
