...
use crate::arch::state::{MachineState, VcpuState};
use crate::config::{MAX_CPUS, MAX_MEM_MIB};
use crate::hv::{Backend, current_backend};
#[cfg(target_arch = "x86_64")]
use crate::layout::PCI_VIRTIO_BAR_SIZE;
use crate::layout::{IRQ_BASE, IRQ_END, MMIO_DEVICE_SIZE, MMIO_MEM_SIZE, MMIO_MEM_START};
use crate::memory::{self, GuestMemoryMmap};
...
pub struct DeltaMemory {
    /// Base snapshot directory basename, resolved against the delta dir's parent.
    pub base: String,
    /// Diff granularity in bytes (always `DELTA_PAGE_SIZE` for written snapshots).
    pub page_size: u64,
...
/// Validate a decoded snapshot image before restore/fork allocates or indexes it.
...
fn parse_generation(contents: &str) -> Result<u64> {
    ...
    if generation.is_empty() || !generation.as_bytes().iter().all(u8::is_ascii_digit) {
        return Err(err(format!(
            "invalid {CURRENT_GENERATION_FILE} contents {generation:?}"
        )));
    }
...
fn validate_snapshot_resource_caps(cpus: u8, mem_mib: usize) -> Result<()> {
    ...
    if cpus > MAX_CPUS {
        return Err(err(format!(
            "snapshot cpus {cpus} exceeds launch cap {MAX_CPUS}"
        )));
    }
    if mem_mib == 0 {
...
fn snapshot_memory_len(path: &Path) -> Result<u64> {
    ...
    if !metadata.is_file() {
        return Err(err(format!(
            "snapshot memory {} is not a regular file",
            path.display()
        )));
    }
    Ok(metadata.len())
...
fn validate_region_layout(snap: &Snapshot) -> Result<u64> {
    ...
    for (idx, region) in snap.mem_regions.iter().enumerate() {
        ...
        if region.len % DELTA_PAGE_SIZE != 0 {
            return Err(err(format!(
                "snapshot memory region {idx} length is not page-aligned"
            )));
        }
        ...
        let end = region.gpa.checked_add(region.len).ok_or_else(|| {
            err(format!(
                "snapshot memory region {idx} guest address range overflows"
            ))
        })?;
        next_file_offset = next_file_offset.checked_add(region.len).ok_or_else(|| {
            err(format!(
                "snapshot memory region {idx} file offset range overflows"
            ))
        })?;
        total_ram = total_ram.checked_add(region.len).ok_or_else(|| {
            err(format!(
                "snapshot memory region {idx} total RAM size overflows"
            ))
        })?;
    ...
    for pair in ranges.windows(2) {
        if pair[1].0 < pair[0].1 {
            return Err(err(format!(
                "snapshot memory regions overlap at {:#x}",
                pair[1].0
            )));
        }
    }
...
fn validate_delta_memory(d: &DeltaMemory, total_ram: u64, memory_len: u64) -> Result<()> {
    if d.page_size != DELTA_PAGE_SIZE {
        return Err(err(format!(
            "snapshot delta page size {} != {DELTA_PAGE_SIZE}",
            d.page_size
        )));
    }
    if !total_ram.is_multiple_of(d.page_size) {
        return Err(err(format!(
            "snapshot delta total RAM {total_ram} is not page-aligned"
        )));
    }
    ...
    if !is_safe_snapshot_name(&d.base) {
        return Err(err(format!(
            "snapshot delta base {:?} is not a safe basename",
            d.base
        )));
    }
    Ok(())
...
fn validate_device_states(devices: &[DeviceState], mem_regions: &[MemRegion]) -> Result<()> {
    for (idx, device) in devices.iter().enumerate() {
        match device.transport {
            DeviceTransportKind::Mmio => {
                if device.transport_pci.is_some() {
                    return Err(err(format!(
                        "snapshot device {idx} has PCI state on MMIO transport"
                    )));
                }
            }
        ...
        match device.kind {
            DeviceKind::Fs => {
                if device.fs.is_none() {
                    return Err(err(format!(
                        "snapshot virtio-fs device {idx} is missing fs state"
                    )));
                }
            }
            _ => {
                if device.fs.is_some() {
                    return Err(err(format!(
                        "snapshot non-fs device {idx} carries virtio-fs state"
                    )));
                }
            }
        }
        let expected_queues = match (&device.kind, &device.backend) {
            ...
            (kind, _) => {
                return Err(err(format!(
                    "snapshot device {idx} backend does not match kind {kind:?}"
                )));
            }
        };
        ...
        for (queue_idx, queue) in device.queues.iter().enumerate() {
            if queue.max_size == 0 {
                return Err(err(format!(
                    "snapshot device {idx} queue {queue_idx} has zero max size"
                )));
            }
...
fn validate_pci_transport_state(idx: usize, device: &DeviceState) -> Result<()> {
    let Some(state) = &device.transport_pci else {
        return Err(err(format!(
            "snapshot PCI device {idx} is missing PCI transport state"
        )));
    };
    ...
    if state.msix.table.is_empty() {
        return Err(err(format!(
            "snapshot PCI device {idx} MSI-X table is empty"
        )));
    }
...
fn validate_device_addressing(devices: &[DeviceState]) -> Result<()> {
    ...
    let gsi_capacity = IRQ_END.checked_sub(IRQ_BASE).ok_or_else(|| {
        err(format!(
            "architecture GSI range {IRQ_BASE}..{IRQ_END} is invalid"
        ))
    })?;
    ...
    for (idx, device) in devices.iter().enumerate() {
        ...
        let mmio_end = device.mmio_base.checked_add(window_size).ok_or_else(|| {
            err(format!(
                "snapshot device {idx} MMIO window overflows guest address space"
            ))
        })?;
        if device.mmio_base < MMIO_MEM_START || mmio_end > aperture_end {
            return Err(err(format!(
                "snapshot device {idx} MMIO window {:#x}..{:#x} is outside device aperture {MMIO_MEM_START:#x}..{aperture_end:#x}",
    ...
    mmio_ranges.sort_unstable_by_key(|&(start, _, _)| start);
    for pair in mmio_ranges.windows(2) {
        let (prev_start, prev_end, prev_idx) = pair[0];
        let (start, end, idx) = pair[1];
        if start < prev_end {
            return Err(err(format!(
                "snapshot device {idx} MMIO window {start:#x}..{end:#x} overlaps device {prev_idx} window {prev_start:#x}..{prev_end:#x}"
            )));
        }
    }
...
fn validate_queue_ram_range(
    ...
    let end = start.checked_add(len).ok_or_else(|| {
        err(format!(
            "snapshot device {device_idx} queue {queue_idx} {label} range overflows guest address space"
        ))
    })?;
    let fits = regions
        .iter()
    ...
    if !fits {
        return Err(err(format!(
            "snapshot device {device_idx} queue {queue_idx} {label} range {start:#x}..{end:#x} is outside declared guest RAM"
        )));
    }
    Ok(())
}
...
fn memory_region_table(mem: &GuestMemoryMmap) -> Result<Vec<MemRegion>> {
    ...
    for region in mem.iter() {
        ...
        if len % DELTA_PAGE_SIZE != 0 {
            return Err(err(format!(
                "guest memory region {gpa:#x} length {len} is not page-aligned"
            )));
        }
...
/// Dump every guest-RAM region into `path` (slot order) and return the region table.
...
fn page_index_to_gpa(page: u64, regions: &[MemRegion]) -> Result<u64> {
    ...
    Err(err(format!(
        "delta page index {page} is outside region table"
    )))
}
...
/// Read changed pages from `memory_file` and apply them to `mem`. Pages are read
/// sequentially in ascending order and mapped to their GPAs via `regions`.
...
/// Resolve a delta's `base` name to a directory path relative to the delta's parent.
...
fn load_layer(
    ...
    if !same_layout(&image.snapshot().mem_regions, expected) {
        return Err(err(
            "delta chain layer RAM layout differs from top snapshot",
        ));
    }
...
/// Reconstruct a (possibly delta) snapshot's guest RAM by loading the full chain.
...
mod tests {
    ...
    fn machine_state() -> MachineState {
        #[cfg(target_os = "linux")]
        let gic = crate::arch::state::GicState::Kvm {
            entries: Vec::new(),
        };
        #[cfg(target_os = "macos")]
    ...
    fn rejects_snapshot_cpus_over_launch_cap() {
        ...
        assert!(
            err.contains(&MAX_CPUS.to_string()),
    ...
    fn rejects_snapshot_mem_over_launch_cap() {
        ...
        assert!(err.contains("launch cap"), "unexpected error: {err}");
        assert!(
            err.contains(&MAX_MEM_MIB.to_string()),
            "unexpected error: {err}"
        );
    }
    ...
    fn rejects_ready_queue_ring_outside_guest_ram() {
        ...
        q.used_ring = 0xfff0;
        let err = validate_ready_queue_ram(
            0,
            0,
            &q,
            &[MemRegion {
    ...
    fn pci_transport_state_survives_serde_round_trip() {
        ...
        device.transport_pci = Some(PciTransportStateSer {
            ...
            msix: MsixStateSer {
                ...
                table: vec![
                    MsixEntrySer {
                        msg_addr: 0xfee0_0000,
                        msg_data: 0x4021,
                        vector_ctrl: 0,
                    },
                    MsixEntrySer {
                        msg_addr: 0xfee0_1000,
                        msg_data: 0x4022,
                        vector_ctrl: 1,
                    },
                ],
