...
//! filesystem/cgroup, unshares the VM namespaces, forks a child that becomes PID
//! 1 in the new PID namespace, and exits with the child's status. The child
//! pivots into the jail, pre-binds operator-owned sockets as root, and then
//! enters the normal VMM lifecycle. Stage-B filters/drop happen after backends open.
...
impl Cgroup {
    ...
    fn setup_v2(config: &Config, id: &str) -> Result<Self> {
        ...
        if !root.join("cgroup.controllers").exists() {
            return Err(err(
                "cgroup v2 is not mounted at /sys/fs/cgroup; use --cgroup-mode off to disable cgroup setup",
            ));
        }
        let path = root.join("vmon").join(id);
        create_dir(&path, 0o755)?;
        write_value(
            &path.join("cpu.max"),
            config.cgroup_cpu_max.as_deref().unwrap_or("max"),
        )?;
...
fn bind_path(chroot: &Path, source: &Path, readonly: bool) -> Result<()> {
    if !source.is_absolute() {
        return Err(err(format!(
            "jail bind source {} must be an absolute path",
            source.display()
        )));
    }
    let meta = fs::symlink_metadata(source).map_err(|e| {
        err(format!(
            "checking jail bind source {}: {e}",
            source.display()
        ))
    })?;
    if meta.file_type().is_symlink() {
        return Err(err(format!(
            "jail bind source {} must not be a symlink",
            source.display()
        )));
    }
...
fn ensure_target_parent(chroot: &Path, path: &Path) -> Result<()> {
    ...
    let parent = target.parent().ok_or_else(|| {
        err(format!(
            "jail path {} must have a parent directory",
            path.display()
        ))
    })?;
