...
INITRAMFS_COMPAT_PATH=${INITRAMFS_COMPAT_PATH:-"$ASSET_DIR/busybox-initramfs.cpio.gz"}
INITRAMFS_VERSION=${INITRAMFS_VERSION:-"2026-06-26.4-$ARCH"}
ROOTFS_COMPAT_PATH=${ROOTFS_COMPAT_PATH:-"$ASSET_DIR/rootfs.ext4"}
...
build_initramfs() {
  ...
  if [ "$INITRAMFS_COMPAT_PATH" != "$INITRAMFS_PATH" ]; then
    ln -sf "$(basename "$INITRAMFS_PATH")" "$INITRAMFS_COMPAT_PATH"
  fi
}
...
build_initramfs
build_rootfs
ln -sf "$(basename "$ROOTFS_PATH")" "$ROOTFS_COMPAT_PATH"
