...
pub mod pause_signal {
    use crate::result::Result;
    use parking_lot::Mutex;
    /// HVF kicks vCPUs via the backend kicker, so no signal is installed.
    pub fn install() -> Result<()> {
        Ok(())
    }
...
impl PauseGate {
    ...
    /// Install a backend-specific wakeup for vCPU APIs that are not signal-driven.
...
/// verifying they are inactive sockets owned by the expected uid or allowed pre-bind uid.
pub fn bind(
    sock_path: PathBuf,
    ...
    match fs::symlink_metadata(parent) {
        ...
        Err(e) if e.kind() == io::ErrorKind::NotFound => {
            fs::DirBuilder::new()
                ...
                .map_err(|e| {
                    err(format!(
                        "creating private control socket directory {}: {e}",
                        parent.display()
                    ))
                })?;
            if let Some(owner) = owner {
...
fn ensure_parent_can_be_reowned(
    ...
    let sock_name = sock_path.file_name().ok_or_else(|| {
        err(format!(
            "control socket path {} must include a file name",
            sock_path.display()
        ))
    })?;
    for entry in fs::read_dir(parent)? {
        ...
        if !socket_owner_is_allowed(meta.uid(), expected_uid, prebind_uid) {
            ...
            return Err(err(format!(
                "refusing to chown control socket parent {} with socket entry owned by uid {}, expected {expected}",
                parent.display(),
                meta.uid()
            )));
        }
...
fn verify_private_parent(
    ...
    if meta.file_type().is_symlink() {
        return Err(err(format!(
            "control socket parent {} must not be a symlink",
            parent.display()
        )));
    }
    if !meta.is_dir() {
...
fn remove_stale_socket(
    ...
    match UnixStream::connect(sock_path) {
        Ok(_) => Err(err(format!(
            "refusing to remove active control socket {}",
            sock_path.display()
        ))),
        Err(e) if e.kind() == io::ErrorKind::ConnectionRefused => Ok(()),
        Err(e) if e.kind() == io::ErrorKind::NotFound => Ok(()),
        Err(e) => Err(err(format!(
            "refusing to probe stale control socket {}: {e}",
            sock_path.display()
        ))),
    }?;
...
fn chown_path(path: &Path, owner: SocketOwner) -> Result<()> {
    let c_path = std::ffi::CString::new(path.as_os_str().as_bytes()).map_err(|_| {
        err(format!(
            "control socket path {} contains an interior NUL byte",
            path.display()
        ))
    })?;
...
fn chmod_path(path: &Path, mode: libc::mode_t) -> Result<()> {
    ...
    if !chmod_fallback_allowed(&e) {
        return Err(err(format!(
            "chmoding control socket path {} to {mode:o}: {e}",
            path.display()
        )));
    }
    ...
    if meta.file_type().is_symlink() {
        return Err(err(format!(
            "refusing to chmod symlink control socket path {}",
            path.display()
        )));
    }
...
fn verify_peer_credentials(stream: &UnixStream, launch_uid: u32) -> io::Result<()> {
    // SAFETY: `cred` is fully initialized by a successful `getsockopt` call; the
    // buffer length is exactly the size of `libc::ucred`.
    ...
    if cred.uid != 0 && cred.uid != launch_uid {
        return Err(io::Error::new(
            ...
            format!(
                "control socket peer uid {} is not root or launch uid {launch_uid}",
                cred.uid
            ),
        ));
    }
...
fn read_request_line(
    ...
    if overlong {
        return Ok(Some(RequestLine::BadRequest(
            "control request line exceeds 65536 bytes",
        )));
    }
    let request = match std::str::from_utf8(line) {
        ...
        Err(_) => {
            return Ok(Some(RequestLine::BadRequest(
                "control request line is not valid UTF-8",
            )));
        }
    };
...
fn parse_request(request: &str) -> std::result::Result<(u64, ControlKind), ApiError> {
    ...
    let kind = match request.method.as_str() {
            ...
            let base = match request.params.get("base") {
                ...
                Some(_) => {
                    return Err(ApiError::bad_request(
                        "snapshot params.base must be a string",
                    ));
                }
            };
        ...
        method => Err(ApiError::unknown_method(format!(
