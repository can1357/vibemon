...
/// KVM vCPU exit reasons we count, mirroring the arms of `run_vcpu`'s exit match.
#[derive(Clone, Copy)]
pub enum VmExit {
    IoIn,
    IoOut,
...
mod tests {
    ...
    fn snapshot_json_reports_recorded_events() {
        ...
        assert_eq!(
            after["vm_exits"]["io_out"].as_u64().unwrap(),
            before_io_out + 2
        );
        assert_eq!(
            after["vm_exits"]["total"].as_u64().unwrap(),
            before_total + 2
        );
        assert_eq!(after["control_requests"].as_u64().unwrap(), before_ctrl + 1);
        assert_eq!(
            after["snapshot"]["count"].as_u64().unwrap(),
            before_snap + 1
        );
