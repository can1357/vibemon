...
pub struct Vmm {
	...
	/// Snapshot whose successor is covered by the current dirty-log window.
...
struct AgentSocket {
	path:     PathBuf,
	listener: crate::windows_pipe::NamedPipeServer,
}
...
pub fn run_inner(config: Config) -> Result<()> {
	run_inner_with_prebound(config, PreboundSockets::none(0))
}
...
fn bind_agent_socket(
	...
	let Some(path) = agent_sock else {
		return Ok(None);
	};
	if path.as_os_str().is_empty() {
		return Err(err("agent pipe path must not be empty"));
	}
	let listener = crate::windows_pipe::NamedPipeServer::bind(&path)
		.map_err(|e| err(format!("binding agent pipe {} failed: {e}", path.display())))?;
	Ok(Some(AgentSocket { path, listener }))
}
...
fn spawn_agent_bridge(
	...
	thread::Builder::new()
		...
		.spawn(move || {
			if let Err(e) =
				agent_bridge_loop(socket, input, input_evt, output, output_evt, drain_resume_evt, gate)
			{
...
fn spawn_agent_bridge(
	...
	let (input, input_evt) = vmm
		...
		.as_ref()
		.ok_or_else(|| err("--agent-sock requires a virtio-console agent device"))?;
	let (output, _output_evt, drain_resume_evt) = vmm
		.console_output
		.as_ref()
		.ok_or_else(|| err("--agent-sock requires guest-output console handles"))?;
	let input = Arc::clone(input);
	...
	thread::Builder::new()
		...
		.spawn(move || {
			if let Err(e) =
				agent_bridge_loop_windows(socket, input, input_evt, output, drain_resume_evt, gate)
			{
				warn!("agent bridge exited: {e}");
			}
		})
...
const fn cleanup_agent_socket(agent_socket: Option<&AgentSocket>) {
	if let Some(agent_socket) = agent_socket {
		let _ = &agent_socket.path;
	}
}
...
fn agent_bridge_loop_windows(
	...
	loop {
		...
		if client.is_none() {
			discard_guest_output(&output, &drain_resume_evt)?;
			client = socket.listener.try_accept()?;
		}
		...
		match stream.read(&mut input_buf) {
			Ok(0) => close = true,
			Ok(n) => {
				console_input.lock().extend(input_buf[..n].iter().copied());
				input_evt.write(1)?;
			},
			Err(e) if e.kind() == io::ErrorKind::WouldBlock => {},
			Err(e) if crate::windows_pipe::NamedPipeClient::disconnected(&e) => close = true,
			Err(_) => close = true,
		}
		if !close && drain_guest_output_to_pipe(stream, &output, &drain_resume_evt).is_err() {
			close = true;
		}
		if close {
			client = None;
...
#[cfg(windows)]
fn drain_guest_output_to_pipe(
	stream: &mut crate::windows_pipe::NamedPipeClient,
	output: &Arc<Mutex<Vec<u8>>>,
	drain_resume_evt: &EventFd,
) -> io::Result<()> {
	loop {
		...
		match stream.write(&out) {
			Ok(0) => {
				return Err(io::Error::new(
					io::ErrorKind::WriteZero,
					"agent pipe accepted zero-byte write",
				));
			},
			Ok(n) => {
				let before = out.len();
	...
	let base_disk: Option<String> = config
		...
		.or_else(|| {
			snap.devices.iter().find_map(|d| match &d.backend {
				...
				BackendHint::Fs { .. } | BackendHint::RemoteFs { .. } | BackendHint::Rng => None,
			})
		});
...
impl Vmm {
	...
	fn build(config: Config) -> Result<Self> {
		...
		let vm = Rc::new(Vm::new(mem_bytes)?);
		if config.ksm {
		...
		let loaded = match boot_mode {
			...
			BootMode::Uefi => {
				...
				{
					arch::boot::load_uefi_firmware(firmware_path, vm.memory(), &vm)?
				}
			},
		};
		...
		if let (Some(tag), Some(dir)) = (&config.fs_tag, &config.fs_dir) {
			...
			if let Some(device) = devices.last_mut() {
				device.fs = Some(FsSave::Host(fs_handle));
			}
			gsi += 1;
		}
		...
		for m in &config.remote_fs {
			...
			match config.transport {
				...
				Transport::Pci => unreachable!("--transport pci is rejected during config parsing"),
			}
			if let Some(device) = devices.last_mut() {
	...
	fn fork_from(dir: &Path, config: &Config) -> Result<Self> {
		...
		let vm = Rc::new(Vm::with_memory(memory)?);
		if config.ksm {
			crate::memory::advise_mergeable(vm.memory());
		}
		Self::reconstruct(vm, image.into_snapshot(), config)
	...
	fn reconstruct(vm: Rc<Vm>, snap: Snapshot, config: &Config) -> Result<Self> {
		...
		let firmware_memory = match boot_mode {
			...
			BootMode::Uefi => {
				let path = snap
					...
					.as_deref()
					.ok_or_else(|| err("UEFI snapshot is missing firmware path"))?;
				Some(arch::boot::map_uefi_firmware(Path::new(path), vm.memory(), &vm)?)
			},
		};
		...
		#[cfg(all(target_os = "windows", target_arch = "x86_64"))]
		arch::state::restore_machine(&vm, &snap.machine)?;
		...
		// Restore register state before the owning threads start.
		#[cfg(any(target_os = "linux", target_os = "windows"))]
		for (id, vcpu) in vcpus.iter().enumerate() {
		...
		if let Some(server) = control_server {
			...
			let deadline_timer = if self.has_pager() {
				let deadline = earliest_deadline(
					...
					self
						.owner_lease_secs
						.map(|secs| deadline_or_now(self.started_at, secs, "--owner-lease-secs")),
				);
				loop {
					if self.gate.state() == RunState::Stopping {
						break None;
	...
	#[allow(
		clippy::unused_self,
		reason = "the VMM run loop calls the platform pager hook uniformly"
	)]
	const fn sweep_pager_if_needed(&self) {}
	...
	#[cfg(target_os = "windows")]
	#[allow(clippy::unused_self, reason = "the VMM run loop queries pager ownership uniformly")]
	const fn has_pager(&self) -> bool {
		false
	}
	...
	/// snapshot. `track` arms WHP dirty logging against the new snapshot.
	fn snapshot(&self, dir: &Path, name: &str, base: Option<&str>, track: bool) -> Result<()> {
		...
		let machine = arch::state::save_machine(&self.vm)?;
		#[cfg(all(target_os = "linux", target_arch = "aarch64"))]
		...
		for d in &self.devices {
			...
			devices.push(DeviceState {
				...
				user_net_state: d.device.lock().user_net_state()?,
				transport_pci,
				fs,
			});
		}
		...
		let tracked_base = {
			let mut dirty_base = self.dirty_base.lock();
			if base.is_some_and(|base| dirty_base.as_deref() == Some(base)) {
				*dirty_base = None;
				true
			} else {
				false
		...
		let guest_dirty = if tracked_base && self.vm.dirty_tracking_available() {
			let vcpu = self
				.vcpus
				.first()
				.ok_or_else(|| err("WHP dirty log requires at least one vCPU"))?;
			Some(vcpu.take_dirty_log(self.vm.memory())?)
		} else {
			None
		};
		...
		if track && self.vm.dirty_tracking_available() {
			let vcpu = self
				.vcpus
				.first()
				.ok_or_else(|| err("WHP dirty tracking requires at least one vCPU"))?;
			vcpu.clear_dirty_log(self.vm.memory())?;
...
fn open_net_device(
	...
	match backend {
		...
		BackendHint::UserNet { mac } => Ok(Arc::new(Mutex::new(Net::new_user_with_state(
			*mac,
...
fn resolve_backend(ds: &DeviceState, config: &Config) -> BackendHint {
	match &ds.backend {
		...
		BackendHint::RemoteFs { tag, sock } => {
			BackendHint::RemoteFs { tag: tag.clone(), sock: sock.clone() }
		},
	}
}
	...
	match boot_mode {
		...
		BootMode::Uefi => {
			arch::boot::configure_uefi_system(
				vm.memory(),
				u8::try_from(vcpus.len()).unwrap(),
				crate::layout::PCI_ECAM_BASE,
				crate::layout::PCI_ECAM_SIZE,
			)?;
			for (id, vcpu) in vcpus.iter().enumerate() {
...
/// The hypervisor run loop for one vCPU. Parks on pause and exits on stop or
/// failure.
