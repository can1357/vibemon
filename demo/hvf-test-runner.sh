...
# (unsigned) `vmm` over any signature we apply beforehand. This runner is
# invoked by cargo *after* that copy, once per test binary, so it ad-hoc signs
# the exact `vmm` the tests spawn and then execs the test.
#
...
test_bin=$1
# binary it spawns (CARGO_BIN_EXE_vmm) is one directory up.
vmm="$(dirname "$test_bin")/../vmm"
