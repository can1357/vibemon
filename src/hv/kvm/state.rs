...
//!
//! The GICv3 register groups, MMIO offsets, conditional active-priority register
//! handling, and the MPIDR -> device-attr packing below are transcribed from
//! Firecracker v1.10.0 (Apache-2.0). The serialized structs live in
//! `arch::state`; this module only captures and replays KVM-specific state.
...
use crate::arch::state::{GicState, MachineState, VcpuState};
use crate::bail;
...
/// frame (`VGIC_SGI_REGS`), in Firecracker's order. `(mmio_offset, size_bytes)`,
/// each accessed in 32-bit chunks.
...
/// without the KVM ONE_REG prefix). Mirrors Firecracker `SimpleReg::vgic_sys_reg`.
...
fn read_attr_val(gic: &DeviceFd, group: u32, attr_id: u64) -> Result<u64> {
    ...
    let mut dev_attr = kvm_device_attr {
        ...
        group,
        attr: attr_id,
        addr: std::ptr::addr_of_mut!(val) as u64,
    };
...
/// Save the CPU-interface (ICC) registers for one vCPU, honoring the implemented
/// active-priority register count (Firecracker `get_icc_regs`).
