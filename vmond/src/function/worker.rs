...
//! immutable revision over protocol v2; it never owns durable queues or results.
...
use super::protocol::{DEFAULT_EVENT_CAPACITY, Frame, ProtocolError, ProtocolSession};
...
impl SecretValues {
	...
	where I: IntoIterator<Item = (K, V)>, K: Into<String>, V: Into<Vec<u8>> {
		Self(items.into_iter().map(|(k, v)| (k.into(), v.into())).collect())
	}

	/// Borrow one secret value without copying it.
	pub fn get(&self, name: &str) -> Option<&[u8]> { self.0.get(name).map(Vec::as_slice) }

	/// Whether material exists for a persisted secret name.
	pub fn contains_key(&self, name: &str) -> bool { self.0.contains_key(name) }

	/// Iterate safe-to-log secret names. Values are never exposed here.
	pub fn names(&self) -> impl Iterator<Item = &str> { self.0.keys().map(String::as_str) }

	/// Whether the set contains no material.
	pub fn is_empty(&self) -> bool { self.0.is_empty() }

	fn sandbox_wire(&self) -> Result<Vec<Value>, WorkerError> {
		self.0.iter().map(|(name, bytes)| {
			let value = std::str::from_utf8(bytes).map_err(|_| WorkerError::platform("invalid_secret", format!("secret {name} is not valid UTF-8 environment data")))?;
			if value.contains('\0') { return Err(WorkerError::platform("invalid_secret", format!("secret {name} contains a NUL byte"))); }
			Ok(json!({"name":name,"values":{name:value}}))
		}).collect()
	}

	fn protocol_wire(&self) -> Value {
		Value::Object(self.0.iter().filter_map(|(name, bytes)| {
			std::str::from_utf8(bytes).ok().map(|value| (name.clone(), Value::String(value.to_owned())))
		}).collect())
	}
	...
	fn redact_error(&self, mut error: WorkerError) -> WorkerError {
		for bytes in self.0.values() {
			if let Ok(secret)=std::str::from_utf8(bytes) {
				if !secret.is_empty() { error.message=error.message.replace(secret,"[REDACTED]"); }
			}
		}
		error
	}
...
impl Drop for SecretValues {
	fn drop(&mut self) {
		for value in self.0.values_mut() { value.fill(0); }
	}
}

/// Stable classification used by retry and actor-loss policy.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum WorkerErrorKind { User, Platform, Infrastructure, ActorLost, Cancelled, Timeout }
...
impl From<ProtocolError> for WorkerError {
	fn from(error: ProtocolError) -> Self {
		match error {
			...
			ProtocolError::Version { .. } | ProtocolError::InvalidFrame(_) => Self::platform("runner_protocol", error.to_string()),
			ProtocolError::Backpressure(_) | ProtocolError::Disconnected(_) => Self::infrastructure("worker_lost", error.to_string()),
		}
	}
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ExecutionMode { Unary, Generator, Batch }
impl ExecutionMode { fn wire(self) -> &'static str { match self { Self::Unary => "unary", Self::Generator => "generator", Self::Batch => "batch" } } }

/// Whether cooperative cancellation can stop this request without killing its VM.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum Interruptibility { Async, Sync }
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum SnapshotReason { PostInitialize, WorkerRetire, ActorCheckpoint }
...
impl SnapshotProvenance {
	...
	pub fn for_revision(revision: &pb::FunctionRevision) -> Result<Self, WorkerError> {
		let revision_id = revision.r#ref.as_ref().map(|r| r.revision_id.clone()).filter(|s| !s.is_empty())
			.ok_or_else(|| WorkerError::platform("invalid_revision", "revision_id is required"))?;
		let spec = revision.spec.as_ref().ok_or_else(|| WorkerError::platform("invalid_revision", "function spec is required"))?;
		let image_digest = spec.image.as_ref().and_then(|i| i.resolved_oci_digest.as_ref()).map(|d| d.value.clone()).unwrap_or_default();
		let package_digest = spec.package.as_ref().and_then(|p| p.content_digest.as_ref()).map(|d| d.value.clone()).unwrap_or_default();
		let initialize_hook = spec.lifecycle_hooks.as_ref().and_then(|hooks| match &hooks.initialize_presence {
			Some(pb::lifecycle_hooks::InitializePresence::Initialize(hook)) => Some((hook.module.clone(), hook.qualname.clone())),
			None => None,
		});
		Ok(Self { revision_id, image_digest, package_digest, initialize_hook, runner_protocol: 2 })
	}

	/// Return true only when every state-bearing immutable input matches.
	pub fn matches(&self, revision: &pb::FunctionRevision) -> bool { Self::for_revision(revision).is_ok_and(|expected| expected == *self) }
}
...
pub trait Executor: Send + Sync {
	fn spawn(&self, revision: Arc<pb::FunctionRevision>, secrets: SecretValues) -> BoxFuture<Result<Arc<dyn Worker>, WorkerError>>;
	fn restore(&self, revision: Arc<pb::FunctionRevision>, snapshot: WorkerSnapshot, secrets: SecretValues) -> BoxFuture<Result<Arc<dyn Worker>, WorkerError>>;
}
...
pub struct EngineExecutor { home: Home, engine: Arc<dyn EngineApi>, serial: AtomicU64 }

impl EngineExecutor {
	/// Construct a shared executor rooted in one daemon home.
	pub fn new(home: Home, engine: Arc<dyn EngineApi>) -> Arc<Self> { Arc::new(Self { home, engine, serial: AtomicU64::new(1) }) }
}
...
impl Executor for EngineExecutor {
	fn spawn(&self, revision: Arc<pb::FunctionRevision>, secrets: SecretValues) -> BoxFuture<Result<Arc<dyn Worker>, WorkerError>> {
		...
		Box::pin(async move { EngineWorker::spawn(home, engine, revision, secrets, serial).await.map(|w| w as Arc<dyn Worker>) })
	}

	fn restore(&self, revision: Arc<pb::FunctionRevision>, snapshot: WorkerSnapshot, secrets: SecretValues) -> BoxFuture<Result<Arc<dyn Worker>, WorkerError>> {
		let engine = Arc::clone(&self.engine);
		let home = self.home.clone();
		Box::pin(async move { EngineWorker::restore(home, engine, revision, snapshot, secrets).await.map(|w| w as Arc<dyn Worker>) })
	}
}
...
impl EngineWorker {
	async fn spawn(home: Home, engine: Arc<dyn EngineApi>, revision: Arc<pb::FunctionRevision>, secrets: SecretValues, serial: u64) -> Result<Arc<Self>, WorkerError> {
		...
		let view = blocking(remaining(startup_deadline)?, { let engine=Arc::clone(&engine); move || engine.create(create) }).await.map_err(|error|secrets.redact_error(error))?;
		...
		let artifact = {
			...
			let source = package.source.as_ref().ok_or_else(|| WorkerError::platform("invalid_revision", "package source is required"))?;
			read_artifact_at(&home, source)?
		};
		let package_sha256=hex::encode(Sha256::digest(&artifact));
		let package_size=artifact.len()as u64;
		let prepared = async {
			blocking(remaining(startup_deadline)?, { let engine=Arc::clone(&engine); let id=id.clone(); move || engine.exec_capture(&id, ExecRequest { cmd: vec!["mkdir".into(), "-p".into(), "/opt/vmon/functions".into(), "/opt/vmon/values".into(), "/run/vmon".into()], ..ExecRequest::default() }) }).await?;
			blocking(remaining(startup_deadline)?, { let engine=Arc::clone(&engine); let id=id.clone(); move || engine.file_write(&id, RUNNER_PATH, RUNNER) }).await?;
			blocking(remaining(startup_deadline)?, { let engine=Arc::clone(&engine); let id=id.clone(); move || engine.file_write(&id, PACKAGE_PATH, &artifact) }).await?;
			let (bootstrap, session) = start_and_connect(Arc::clone(&engine), &id, remaining(startup_deadline)?).await?;
		...
		let initialized=async {
			...
			if let Some(hook) = initialize_hook(spec) { worker.run_named_hook("initialize", hook, remaining(startup_deadline)?).await?; }
			if spec.lifecycle_hooks.as_ref().is_some_and(|h| h.snapshot_after_initialize) {
				let snapshot = tokio::time::timeout(remaining(startup_deadline)?,worker.capture_snapshot(SnapshotReason::PostInitialize)).await.map_err(|_|WorkerError::timeout("startup"))??;
				*worker.initial_snapshot.write() = Some(snapshot);
			}
			Ok::<(),WorkerError>(())
		}.await;
		if let Err(error)=initialized{let engine=Arc::clone(&worker.engine);let id=worker.id.clone();let _=tokio::task::spawn_blocking(move||engine.remove(&id)).await;return Err(error);}
		Ok(worker)
	}

	async fn restore(home: Home, engine: Arc<dyn EngineApi>, revision: Arc<pb::FunctionRevision>, snapshot: WorkerSnapshot, secrets: SecretValues) -> Result<Arc<Self>, WorkerError> {
		...
		let started=Instant::now();
		let startup_deadline=started+startup;
		let view = blocking(remaining(startup_deadline)?, { let engine=Arc::clone(&engine); let name=snapshot.engine_snapshot.clone(); move || engine.restore(&name, RestoreBody { agent:Some(true), extra, ..RestoreBody::default() }) }).await.map_err(|error|secrets.redact_error(error))?;
		let id=sandbox_id(&view).ok_or_else(|| WorkerError::infrastructure("restore_failed", "restored sandbox view omitted identity"))?;
		let session=connect_existing(Arc::clone(&engine), &id, remaining(startup_deadline)?).await?;
		...
		let expected_package=spec.package.as_ref().and_then(|package|package.source.as_ref()).and_then(|source|source.digest.as_ref()).ok_or_else(||WorkerError::platform("invalid_revision","package source digest is required"))?;
	...
	async fn define(&self, timeout: Duration) -> Result<(), WorkerError> {
		...
		let callable_kind=terminal.0.get("callable_kind").and_then(Value::as_str).unwrap_or("sync");
		self.async_interruptible.store(callable_interruptibility(callable_kind)==Interruptibility::Async,Ordering::Release);
		Ok(())
	}


	async fn run_named_hook(&self, phase:&str, hook:&pb::LifecycleHookRef, timeout:Duration)->Result<(),WorkerError>{
		if !worker_hook_allowed(self.revision.spec.as_ref(),hook)?{return Ok(());}
	...
	async fn capture_snapshot(&self, reason:SnapshotReason)->Result<WorkerSnapshot,WorkerError>{
		...
		if let Some(hook)=snapshot_hook(self.revision.spec.as_ref()).cloned(){self.run_named_hook("snapshot",&hook,timeout).await?;}
		self.control("before_snapshot", timeout).await?;
		let view=blocking_phase(timeout,"snapshot",{let engine=Arc::clone(&self.engine);let id=self.id.clone();move||engine.snapshot(&id,None,false)}).await?;
		let name=view.get("snapshot").and_then(Value::as_str).ok_or_else(||WorkerError::infrastructure("snapshot_failed","engine omitted snapshot name"))?.to_owned();
		Ok(WorkerSnapshot{engine_snapshot:name,provenance:SnapshotProvenance::for_revision(&self.revision)?,reason,created_at_unix_millis:now_ms()})
	}
	...
	fn wire_input(&self, envelope: &pb::ValueEnvelope) -> Result<(Value,Option<String>), WorkerError> {
		...
		let guest_path = if let Some(pb::value_envelope::Storage::Artifact(reference)) = &envelope.storage {
			...
			self.engine.file_write(&self.id, &path, &bytes).map_err(engine_error)?;
			Some(path)
		} else {
			None
		};
	...
	fn execute_frame(&self, frame:Value, request_id:String, interruptibility:Interruptibility, attempt:u32, cleanup_paths:Vec<String>)->Result<Execution,WorkerError>{
		if self.closed.load(Ordering::Acquire){cleanup_guest_paths(self.engine.as_ref(),&self.id,&cleanup_paths);return Err(WorkerError::infrastructure("worker_retired","worker is retired"));}
		{
			let mut active=self.active.lock();
			if active.len()>=self.capacity{cleanup_guest_paths(self.engine.as_ref(),&self.id,&cleanup_paths);return Err(WorkerError::infrastructure("worker_busy","worker concurrency is exhausted"));}
			if active.insert(request_id.clone(),interruptibility).is_some(){cleanup_guest_paths(self.engine.as_ref(),&self.id,&cleanup_paths);return Err(WorkerError::platform("duplicate_request","request is already active"));}
		}
		let protocol=match self.session.request(frame){Ok(request)=>request,Err(error)=>{self.active.lock().remove(&request_id);cleanup_guest_paths(self.engine.as_ref(),&self.id,&cleanup_paths);return Err(error.into());}};
		...
		let event_worker=match std::thread::Builder::new().name(format!("vmon-events-{request_id}")).spawn(move||{while let Ok(mut frame)=raw_events.recv(){if frame.event()==Some("yield"){if let Err(error)=materialize_output(event_engine.as_ref(),&event_worker_id,&mut frame){*event_failure.lock()=Some(error);break;}}if let Some(event)=typed_event(&frame){if events_tx.try_send(event).is_err(){*event_failure.lock()=Some(WorkerError::infrastructure("event_backpressure","worker event consumer did not keep up"));break;}}}}){Ok(worker)=>worker,Err(error)=>{self.active.lock().remove(&request_id);let _=self.session.cancel(&request_id);cleanup_guest_paths(self.engine.as_ref(),&self.id,&cleanup_paths);return Err(WorkerError::infrastructure("event_dispatch",error.to_string()));}};
		let active=Arc::clone(&self.active);let engine=Arc::clone(&self.engine);let id=self.id.clone();let startup=self.startup_millis;let restored=self.restored;let timeout=execution_timeout(&self.revision);let session=self.session.clone();let closed=Arc::clone(&self.closed);
		let completion=Box::pin(async move{
			let started=Instant::now();let mut result=protocol.complete(timeout).await;active.lock().remove(&request_id);
			if matches!(&result,Err(ProtocolError::Timeout)){
				match interruptibility {
					Interruptibility::Async=>{let _=session.cancel(&request_id);}
					Interruptibility::Sync=>{closed.store(true,Ordering::Release);let _=session.kill();let terminate=Arc::clone(&engine);let worker=id.clone();let _=tokio::task::spawn_blocking(move||terminate.terminate(&worker,"synchronous function deadline")).await;}
				}
			}
			let _=tokio::task::spawn_blocking(move||event_worker.join()).await;
			let cleanup_engine=Arc::clone(&engine);let cleanup_id=id.clone();
			let _=tokio::task::spawn_blocking(move||cleanup_guest_paths(cleanup_engine.as_ref(),&cleanup_id,&cleanup_paths)).await;
			if let Some(error)=event_error.lock().take(){result=Err(ProtocolError::Disconnected(error.to_string()));}
			if let Ok(mut frame)=result {
				let output_engine=Arc::clone(&engine);let output_id=id.clone();
				result=match tokio::task::spawn_blocking(move||{materialize_output(output_engine.as_ref(),&output_id,&mut frame)?;Ok::<Frame,WorkerError>(frame)}).await {
					Ok(Ok(frame))=>Ok(frame),
					Ok(Err(error))=>return Ok(WorkerOutcome::PlatformError{error,stats:AttemptStats{attempt,startup_millis:startup,execution_millis:millis(started.elapsed()),snapshot_restore:restored,..AttemptStats::default()}}),
					Err(error)=>return Err(WorkerError::infrastructure("result_materialization",error.to_string())),
				};
			}
			let mut stats=AttemptStats{attempt,startup_millis:startup,execution_millis:millis(started.elapsed()),snapshot_restore:restored,..AttemptStats::default()};
			if let Ok(metrics)=tokio::task::spawn_blocking(move||engine.metrics(&id)).await{if let Ok(metrics)=metrics{stats.cpu_millis=metrics.get("cpu_millis").and_then(Value::as_u64).unwrap_or(0);stats.peak_memory_bytes=metrics.get("peak_memory_bytes").or_else(||metrics.get("memory_bytes")).and_then(Value::as_u64).unwrap_or(0);}}
			match result {Ok(frame)=>Ok(outcome(frame,stats)),Err(error)=>{let error:WorkerError=error.into();Ok(match error.kind{WorkerErrorKind::Infrastructure=>WorkerOutcome::InfrastructureError{error,stats},WorkerErrorKind::ActorLost=>WorkerOutcome::ActorLost{error,stats},_=>WorkerOutcome::PlatformError{error,stats}})}}
		});
...
impl Worker for EngineWorker {
	...
	fn revision_id(&self)->&str{&self.revision_id}
	fn interruptibility(&self)->Interruptibility{if self.async_interruptible.load(Ordering::Acquire){Interruptibility::Async}else{Interruptibility::Sync}}
	...
	fn cancel(&self,request_id:&str)->BoxFuture<Result<(),WorkerError>>{
		...
		Box::pin(async move{match mode{Some(Interruptibility::Async)=>session.cancel(&request_id).map_err(Into::into),Some(Interruptibility::Sync)=>{closed.store(true,Ordering::Release);session.kill()?;let _=tokio::task::spawn_blocking(move||engine.terminate(&id,"synchronous function cancellation")).await;Ok(())},None=>Ok(())}})
	}
	fn snapshot(&self,reason:SnapshotReason)->BoxFuture<Result<WorkerSnapshot,WorkerError>>{
		let engine=Arc::clone(&self.engine);let id=self.id.clone();let session=self.session.clone();let revision=Arc::clone(&self.revision);let hook=snapshot_hook(revision.spec.as_ref()).cloned();
		let hook=match filter_worker_hook(revision.spec.as_ref(),hook){Ok(hook)=>hook,Err(error)=>return Box::pin(async move{Err(error)})};
		let package_sha256=self.package_sha256.clone();let package_size=self.package_size;
		...
		Box::pin(async move{
			...
			terminal_error(&session.request(json!({"type":"before_snapshot","request_id":"lifecycle:before_snapshot"}))?.complete(timeout).await?)?;
			let view=blocking_phase(timeout,"snapshot",{let engine=Arc::clone(&engine);let id=id.clone();move||engine.snapshot(&id,None,false)}).await?;
			let name=view.get("snapshot").and_then(Value::as_str).ok_or_else(||WorkerError::infrastructure("snapshot_failed","engine omitted snapshot name"))?.to_owned();
			Ok(WorkerSnapshot{engine_snapshot:name,provenance:SnapshotProvenance::for_revision(&revision)?,reason,created_at_unix_millis:now_ms()})
		})
	}
	fn actor(&self,request:ActorRequest)->BoxFuture<Result<Execution,WorkerError>>{
		let result=(||{
			...
			let (kind,extra)=match request.operation{
				...
				ActorOperation::Checkpoint{checkpoint_id}=>("actor_checkpoint",json!({"checkpoint_id":checkpoint_id})),
				ActorOperation::Restore{checkpoint_id,state}=>{let(state,path)=self.wire_input(&state)?;cleanup_paths.extend(path);("actor_restore",json!({"checkpoint_id":checkpoint_id,"state":state}))},
	...
	fn retire(&self)->BoxFuture<Result<(),WorkerError>>{
		...
		let secrets=self.secrets.protocol_wire();
		Box::pin(async move{
			if closed.swap(true,Ordering::AcqRel){return Ok(());}
			...
			let result=tokio::time::timeout(timeout,tokio::task::spawn_blocking(move||engine.remove(&id))).await;
			match result{Ok(Ok(Ok(_)))=>Ok(()),Ok(Ok(Err(e)))=>Err(engine_error(e)),Ok(Err(e))=>Err(WorkerError::infrastructure("worker_teardown",e.to_string())),Err(_)=>Err(WorkerError::timeout("shutdown"))}
		})
	}
	fn initial_snapshot(&self)->Option<WorkerSnapshot>{self.initial_snapshot.read().clone()}
}

fn sandbox_create(spec:&pb::FunctionSpec,name:&str,secrets:&SecretValues)->Result<SandboxCreate,WorkerError>{
	let image=spec.image.as_ref().ok_or_else(||WorkerError::platform("invalid_image","image is required"))?;let mut create=SandboxCreate{name:Some(name.to_owned()),secrets:Some(secrets.sandbox_wire()?),env:Some(image.environment.clone()),..SandboxCreate::default()};
	match image.source.as_ref().ok_or_else(||WorkerError::platform("invalid_image","image source is required"))?{pb::image_spec::Source::Python(source)=>create.image=Some(format!("python:{}-{}",source.python_version,source.variant)),pb::image_spec::Source::Registry(source)=>create.image=Some(source.reference.clone()),pb::image_spec::Source::Template(source)=>create.template=Some(format!("{}@{}",source.name,source.revision)),pb::image_spec::Source::Dockerfile(source)=>{create.dockerfile=Some(source.dockerfile_path.clone());create.context=source.context.as_ref().and_then(|r|r.digest.as_ref()).map(|d|format!("sha256:{}",hex::encode(&d.value))).unwrap_or_default();}}
	if let Some(resources)=&spec.resources{create.cpus=((resources.cpu_millis.max(1)+999)/1000).max(1);create.memory=bytes_to_mib(resources.memory_bytes,512);create.disk_mb=bytes_to_mib(resources.ephemeral_disk_bytes,1024);create.arch=match pb::CpuArchitecture::try_from(resources.architecture).unwrap_or(pb::CpuArchitecture::Unspecified){pb::CpuArchitecture::Amd64=>Some("amd64".into()),pb::CpuArchitecture::Arm64=>Some("arm64".into()),pb::CpuArchitecture::Unspecified=>None};create.ha=match pb::HighAvailabilityPolicy::try_from(resources.high_availability).unwrap_or(pb::HighAvailabilityPolicy::Unspecified){pb::HighAvailabilityPolicy::None=>Some("none".into()),pb::HighAvailabilityPolicy::Host=>Some("host".into()),pb::HighAvailabilityPolicy::Zone=>Some("zone".into()),pb::HighAvailabilityPolicy::Unspecified=>None};let mut volumes=HashMap::new();for mount in &resources.volume_mounts{validate_absolute(&mount.mount_path)?;let volume=mount.volume.as_ref().ok_or_else(||WorkerError::platform("invalid_volume","volume reference missing"))?;volumes.insert(mount.mount_path.clone(),json!({"name":volume.name,"read_only":mount.read_only}));}if !volumes.is_empty(){create.volumes=Some(volumes);}if let Some(network)=&resources.network{create.block_network=network.block_network;create.egress_allow=Some(network.egress_cidrs.clone());create.egress_allow_domains=Some(network.egress_domains.clone());create.inbound_cidr_allowlist=Some(network.inbound_cidrs.clone());}}
	create.timeout_secs=spec.timeouts.as_ref().map(|t|((t.execution_millis+t.startup_millis+999)/1000).max(1));Ok(create)
}

async fn start_and_connect(engine:Arc<dyn EngineApi>,id:&str,timeout:Duration)->Result<(ExecStream,ProtocolSession),WorkerError>{
	let bootstrap=blocking(timeout,{let engine=Arc::clone(&engine);let id=id.to_owned();move||engine.exec_stream(&id,ExecRequest{cmd:vec!["python3".into(),RUNNER_PATH.into(),"--socket".into(),SOCKET_PATH.into()],..ExecRequest::default()})}).await?;
	let session=connect_existing(engine,id,timeout).await?;
	Ok((bootstrap,session))
}
async fn connect_existing(engine:Arc<dyn EngineApi>,id:&str,timeout:Duration)->Result<ProtocolSession,WorkerError>{
	let relay_code="import os,socket,select,sys;s=socket.socket(socket.AF_UNIX);s.connect(sys.argv[1]);fds=[0,s.fileno()];\nwhile True:\n r,_,_=select.select(fds,[],[])\n if 0 in r:\n  b=os.read(0,65536)\n  if not b: break\n  s.sendall(b)\n if s.fileno() in r:\n  b=s.recv(65536)\n  if not b: break\n  os.write(1,b)";
	let deadline=Instant::now()+timeout;
	loop{
		let stream=blocking(remaining(deadline)?.min(Duration::from_secs(5)),{let engine=Arc::clone(&engine);let id=id.to_owned();let code=relay_code.to_owned();move||engine.exec_stream(&id,ExecRequest{cmd:vec!["python3".into(),"-c".into(),code,SOCKET_PATH.into()],..ExecRequest::default()})}).await?;
		match ProtocolSession::connect(stream,deadline.saturating_duration_since(Instant::now()),DEFAULT_EVENT_CAPACITY).await{
			Ok(session)=>return Ok(session),
			Err(ProtocolError::Disconnected(_)|ProtocolError::Timeout) if Instant::now()<deadline=>tokio::time::sleep(Duration::from_millis(25)).await,
			Err(e)=>return Err(e.into())
		}
	}
}

async fn blocking<T:Send+'static>(timeout:Duration,operation:impl FnOnce()->crate::Result<T>+Send+'static)->Result<T,WorkerError>{blocking_phase(timeout,"startup",operation).await}
...
fn materialize_output(engine:&dyn EngineApi,worker_id:&str,frame:&mut Frame)->Result<(),WorkerError>{
	...
	let Some(path)=value.get("path").and_then(Value::as_str).map(str::to_owned) else{return Ok(());};
	let allowed=path.starts_with("/run/vmon/results/")||path.starts_with("/run/vmon/values/");
	if !allowed||Path::new(&path).components().any(|part|matches!(part,Component::ParentDir)){return Err(WorkerError::platform("invalid_spill_path","runner returned an unsafe spill path"));}
	let read=engine.file_read(worker_id,&path).map_err(engine_error);
	let delete=engine.file_delete(worker_id,&path,false).map_err(engine_error);
	let bytes=read?;
	delete?;
	if bytes.len()>64*1024*1024{return Err(WorkerError::platform("result_too_large","runner spill exceeds 64 MiB"));}
	value["inline_data"]=json!(BASE64.encode(bytes));
	if let Some(object)=value.as_object_mut(){object.remove("path");object.remove("remove_after_read");}
	Ok(())
}
...
pub fn envelope_to_wire(envelope: &pb::ValueEnvelope, artifact_path: Option<&str>) -> Result<Value, WorkerError> {
	let format = match pb::ValueSerializer::try_from(envelope.serializer).unwrap_or(pb::ValueSerializer::Unspecified) {
		pb::ValueSerializer::Json => "json",
		pb::ValueSerializer::Cbor => "cbor",
		pb::ValueSerializer::Cloudpickle => "cloudpickle",
		pb::ValueSerializer::Unspecified => return Err(WorkerError::platform("invalid_value", "value serializer is unspecified")),
	};
	let compression = match pb::ValueCompression::try_from(envelope.compression).unwrap_or(pb::ValueCompression::None) {
		pb::ValueCompression::None => "none",
		pb::ValueCompression::Gzip => "gzip",
		pb::ValueCompression::Zstd => "zstd",
	};
	let checksum = envelope.checksum.as_ref().ok_or_else(|| WorkerError::platform("invalid_value", "value checksum is required"))?;
	...
	match &envelope.storage {
		...
		Some(pb::value_envelope::Storage::Artifact(_)) => {
			let path = artifact_path.ok_or_else(|| WorkerError::platform("artifact_unavailable", "artifact value was not materialized in the guest"))?;
...
/// Convert a runner protocol-v2 envelope back into the exact protobuf value contract.
pub fn wire_to_envelope(wire: &Value) -> Result<pb::ValueEnvelope, WorkerError> {
	let format = wire.get("format").and_then(Value::as_str).ok_or_else(|| WorkerError::platform("invalid_runner_value", "runner value format is missing"))?;
	let serializer = match format {"json"=>pb::ValueSerializer::Json,"cbor"=>pb::ValueSerializer::Cbor,"cloudpickle"=>pb::ValueSerializer::Cloudpickle,_=>return Err(WorkerError::platform("invalid_runner_value",format!("unsupported runner value format {format}")))};
	let compression = match wire.get("compression").and_then(Value::as_str).unwrap_or("none") {"none"=>pb::ValueCompression::None,"gzip"=>pb::ValueCompression::Gzip,"zstd"=>pb::ValueCompression::Zstd,other=>return Err(WorkerError::platform("invalid_runner_value",format!("unsupported runner compression {other}")))};
	let encoded=wire.get("inline_data").and_then(Value::as_str).ok_or_else(||WorkerError::platform("invalid_runner_value","runner result must be inline"))?;
	let bytes=BASE64.decode(encoded).map_err(|_|WorkerError::platform("invalid_runner_value","runner result contains invalid base64"))?;
	if compression!=pb::ValueCompression::None{return Err(WorkerError::platform("invalid_runner_value","runner outputs must be uncompressed"));}
	let checksum_text=wire.get("sha256").and_then(Value::as_str).ok_or_else(||WorkerError::platform("invalid_runner_value","runner result checksum is missing"))?;
	let checksum=hex::decode(checksum_text.strip_prefix("sha256:").unwrap_or(checksum_text)).map_err(|_|WorkerError::platform("invalid_runner_value","runner result checksum is invalid"))?;
	if checksum.len()!=32{return Err(WorkerError::platform("invalid_runner_value","runner result checksum is not SHA-256"));}
...
fn typed_event(frame:&Frame)->Option<WorkerEvent>{match frame.event()?{"log"=>Some(WorkerEvent::Log{stream:frame.0.get("stream").and_then(Value::as_str).unwrap_or("stdout").to_owned(),message:frame.0.get("message").and_then(Value::as_str).unwrap_or("").to_owned()}),"yield"=>wire_to_envelope(frame.0.get("value")?).ok().map(|value|WorkerEvent::Yield{index:frame.0.get("index").and_then(Value::as_u64).unwrap_or(0),value}),"status"=>Some(WorkerEvent::Status{status:frame.0.get("status").and_then(Value::as_str).unwrap_or("").to_owned()}),_=>None}}
fn callable_interruptibility(kind:&str)->Interruptibility{if matches!(kind,"async"|"async_generator"){Interruptibility::Async}else{Interruptibility::Sync}}

fn read_artifact_at(home:&Home,reference:&pb::ArtifactRef)->Result<Vec<u8>,WorkerError>{let digest=reference.digest.as_ref().ok_or_else(||WorkerError::platform("invalid_artifact","artifact digest required"))?;if digest.value.len()!=32{return Err(WorkerError::platform("invalid_artifact","artifact digest must be SHA-256"));}let hex=hex::encode(&digest.value);let path=home.root().join("functions/artifacts").join(&hex[..2]).join(&hex[2..]);let bytes=std::fs::read(path).map_err(|_|WorkerError::platform("artifact_unavailable",format!("artifact {hex} is unavailable")))?;if Sha256::digest(&bytes).as_slice()!=digest.value.as_slice(){return Err(WorkerError::platform("artifact_corrupt","artifact digest mismatch"));}Ok(bytes)}
fn worker_capacity(spec:&pb::FunctionSpec)->usize{let concurrency=spec.concurrency.as_ref().map_or(1,|c|c.max_concurrent_calls.max(1));let outstanding=spec.workers.as_ref().map_or(concurrency,|w|w.max_outstanding_inputs.max(concurrency));outstanding.min(concurrency)as usize}
fn duration_ms(value:u64,default:Duration)->Duration{if value==0{default}else{Duration::from_millis(value)}}
fn remaining(deadline:Instant)->Result<Duration,WorkerError>{let duration=deadline.saturating_duration_since(Instant::now());if duration.is_zero(){Err(WorkerError::timeout("startup"))}else{Ok(duration)}}
fn execution_timeout(revision:&pb::FunctionRevision)->Duration{duration_ms(revision.spec.as_ref().and_then(|s|s.timeouts.as_ref()).map_or(0,|t|t.execution_millis),DEFAULT_EXECUTION)}
fn shutdown_timeout(revision:&pb::FunctionRevision)->Duration{duration_ms(revision.spec.as_ref().and_then(|s|s.timeouts.as_ref()).map_or(0,|t|t.graceful_shutdown_millis),DEFAULT_SHUTDOWN)}
fn initialize_hook(spec:&pb::FunctionSpec)->Option<&pb::LifecycleHookRef>{spec.lifecycle_hooks.as_ref().and_then(|h|match &h.initialize_presence{Some(pb::lifecycle_hooks::InitializePresence::Initialize(h))=>Some(h),None=>None})}
fn shutdown_hook(spec:Option<&pb::FunctionSpec>)->Option<&pb::LifecycleHookRef>{spec.and_then(|s|s.lifecycle_hooks.as_ref()).and_then(|h|match &h.shutdown_presence{Some(pb::lifecycle_hooks::ShutdownPresence::Shutdown(h))=>Some(h),None=>None})}
fn restore_hook(spec:Option<&pb::FunctionSpec>)->Option<&pb::LifecycleHookRef>{spec.and_then(|s|s.lifecycle_hooks.as_ref()).and_then(|h|match &h.restore_presence{Some(pb::lifecycle_hooks::RestorePresence::Restore(h))=>Some(h),None=>None})}
fn snapshot_hook(spec:Option<&pb::FunctionSpec>)->Option<&pb::LifecycleHookRef>{spec.and_then(|s|s.lifecycle_hooks.as_ref()).and_then(|h|match &h.snapshot_presence{Some(pb::lifecycle_hooks::SnapshotPresence::Snapshot(h))=>Some(h),None=>None})}
fn worker_hook_allowed(spec:Option<&pb::FunctionSpec>,hook:&pb::LifecycleHookRef)->Result<bool,WorkerError>{if hook.module.is_empty()||hook.qualname.is_empty(){return Err(WorkerError::platform("invalid_lifecycle_hook","lifecycle hook module and qualname are required"));}if !hook.qualname.contains('.')&&!hook.qualname.contains("<locals>"){return Ok(true);}let lifecycle=spec.map(|spec|pb::FunctionLifecycle::try_from(spec.lifecycle).unwrap_or(pb::FunctionLifecycle::Unspecified)).unwrap_or(pb::FunctionLifecycle::Unspecified);if matches!(lifecycle,pb::FunctionLifecycle::Actor|pb::FunctionLifecycle::Instance){Ok(false)}else{Err(WorkerError::platform("invalid_lifecycle_hook","worker-level lifecycle hooks must be module functions"))}}
fn filter_worker_hook(spec:Option<&pb::FunctionSpec>,hook:Option<pb::LifecycleHookRef>)->Result<Option<pb::LifecycleHookRef>,WorkerError>{match hook{Some(hook)if worker_hook_allowed(spec,&hook)?=>Ok(Some(hook)),_=>Ok(None)}}
fn bytes_to_mib(bytes:u64,default:u32)->u32{if bytes==0{return default;}((bytes.saturating_add(1024*1024-1)/(1024*1024)).min(u32::MAX as u64))as u32}
fn sandbox_id(view:&Value)->Option<String>{view.get("name").or_else(||view.get("id")).and_then(Value::as_str).map(str::to_owned)}
fn sanitize_name(value:&str)->String{let clean:String=value.chars().map(|c|if c.is_ascii_alphanumeric()||c=='-'{c}else{'-'}).take(32).collect();if clean.is_empty(){"revision".into()}else{clean}}
fn validate_absolute(path:&str)->Result<(),WorkerError>{let path=Path::new(path);if !path.is_absolute()||path.components().any(|c|matches!(c,Component::ParentDir)){return Err(WorkerError::platform("invalid_path",format!("guest mount path {path:?} must be absolute and normalized")));}Ok(())}
fn millis(duration:Duration)->u64{duration.as_millis().min(u64::MAX as u128)as u64}
fn system_ms(time:SystemTime)->u64{time.duration_since(UNIX_EPOCH).unwrap_or_default().as_millis().min(u64::MAX as u128)as u64}
fn now_ms()->u64{system_ms(SystemTime::now())}
...
mod tests {
	...
	fn revision() -> pb::FunctionRevision {
		pb::FunctionRevision {
			...
			spec: Some(pb::FunctionSpec {
				package: Some(pb::PackageSpec { content_digest: Some(digest(1)), ..pb::PackageSpec::default() }),
				image: Some(pb::ImageSpec { resolved_oci_digest: Some(digest(2)), source: Some(pb::image_spec::Source::Registry(pb::RegistryImageSource { reference: "example@sha256:00".into() })), ..pb::ImageSpec::default() }),
				lifecycle_hooks: Some(pb::LifecycleHooks {
					initialize_presence: Some(pb::lifecycle_hooks::InitializePresence::Initialize(pb::LifecycleHookRef { module: "hooks".into(), qualname: "initialize".into() })),
	...
	fn protobuf_envelope_round_trips_protocol_wire() {
		...
		let envelope = pb::ValueEnvelope {
			...
			checksum: Some(pb::Digest { algorithm: pb::DigestAlgorithm::Sha256 as i32, value: Sha256::digest(&bytes).to_vec() }),
	...
	fn snapshot_provenance_invalidates_on_immutable_input_change() {
		...
		changed.spec.as_mut().unwrap().image.as_mut().unwrap().resolved_oci_digest = Some(digest(9));
	...
	fn sandbox_mapping_is_typed_and_secret_errors_are_redacted() {
		...
		let create = sandbox_create(revision().spec.as_ref().unwrap(), "worker", &secrets).unwrap();
		assert_eq!(create.image.as_deref(), Some("example@sha256:00"));
		assert_eq!(create.secrets.as_ref().unwrap()[0]["values"]["TOKEN"], "sensitive-value");
		let redacted=secrets.redact_error(WorkerError::infrastructure("engine","create failed with sensitive-value"));
		assert_eq!(redacted.message,"create failed with [REDACTED]");
		let mut invalid = SecretValues::new();
		invalid.insert("TOKEN", vec![0xff, 0xfe]);
		let error = sandbox_create(revision().spec.as_ref().unwrap(), "worker", &invalid).unwrap_err();
	...
	fn concurrent_artifact_inputs_get_unique_confined_paths() {
		...
		assert!(paths.iter().all(|path|path.starts_with("/opt/vmon/values/")&&path.len()=="/opt/vmon/values/".len()+32));
		let artifact=pb::ValueEnvelope{schema_version:1,serializer:pb::ValueSerializer::Json as i32,compression:pb::ValueCompression::None as i32,checksum:Some(digest(4)),uncompressed_size_bytes:2*1024*1024,storage:Some(pb::value_envelope::Storage::Artifact(pb::ArtifactRef{digest:Some(digest(4))})),python_presence:None,type_name_presence:None};
		for path in &paths {
	...
	fn actor_class_hooks_are_runner_instance_owned() {
		...
		let class_hook=pb::LifecycleHookRef{module:"service".into(),qualname:"Counter.enter".into()};
		assert!(!worker_hook_allowed(Some(spec),&class_hook).unwrap());
		let module_hook=pb::LifecycleHookRef{module:"service".into(),qualname:"initialize".into()};
		assert!(worker_hook_allowed(Some(spec),&module_hook).unwrap());
		spec.lifecycle=pb::FunctionLifecycle::Stateless as i32;
		assert_eq!(worker_hook_allowed(Some(spec),&class_hook).unwrap_err().code,"invalid_lifecycle_hook");
	}

	#[test]
	fn unavailable_actor_is_explicit_actor_lost() {
		let frame = Frame(json!({"protocol":2,"type":"error","request_id":"actor","error":{"phase":"actor","type":"KeyError","message":"actor is unavailable; explicit restore is required"}}));
