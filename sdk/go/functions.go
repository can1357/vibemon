...
type callOptions struct { labels map[string]string; resultTTL *uint64; codec ValueCodec; compression ValueCompression }
...
func (function *Function[T]) Spawn(ctx context.Context, input any) (*FunctionCall[T], error) { return function.spawn(ctx, pb.CallType_CALL_TYPE_UNARY, []any{input}, true) }
...
func (function *Function[T]) Remote(ctx context.Context, input any) (T, error) {
	call, err := function.spawn(ctx, pb.CallType_CALL_TYPE_UNARY, []any{input}, true); if err != nil { var zero T; return zero, err }
	return call.Get(ctx)
}
...
func (function *Function[T]) spawn(ctx context.Context, kind pb.CallType, values []any, closed bool) (*FunctionCall[T], error) {
	...
	for index, value := range values { envelope, err := EncodeValue(value, codec, function.options.compression); if err != nil { return nil, err }; inputs[index] = &pb.CallInput{Index: uint64(index), InputId: fmt.Sprintf("%d", index), Payload: &pb.CallInput_Value{Value: envelope.wire}} }
...
type FunctionCall[T any] struct { client *Client; id string; endpoint string }
...
type AttemptStats struct { AttemptID string; UserAttempt, InfraAttempt uint32; QueuedMillis, StartupMillis, ExecutionMillis, CPUMillis, PeakMemoryBytes uint64 }
...
type CallGraph struct { Parents []ParentEdge; RootCallID *string }
...
func (call *FunctionCall[T]) Graph(ctx context.Context) (*CallGraph, error) { record, err := call.record(ctx); if err != nil { return nil, err };if record.Graph==nil{return nil,nil};result:=&CallGraph{};for _,parent:=range record.Graph.Parents{result.Parents=append(result.Parents,ParentEdge{CallID:parent.CallId,InputID:parent.InputId})};if record.Graph.GetRootCallIdPresence()!=nil{root:=record.Graph.GetRootCallId();result.RootCallID=&root};return result,nil }
		...
		for {
			...
			if err == nil {
				...
				for {
					...
					if recvErr != nil { err = recvErr; break }
			...
			if !follow && err == io.EOF { return }
			if ctx.Err() != nil { failures <- ctx.Err(); return }
			hint = ""
			select {
			case <-time.After(50 * time.Millisecond):
			case <-ctx.Done(): failures <- ctx.Err(); return
			}
		}
	}()
...
func projectCallEvent(event *pb.CallEvent) CallEvent {
	item := CallEvent{Sequence:event.Sequence, Kind:CallEventKind(event.Type)}
	if event.GetInputIdPresence()!=nil { value:=event.GetInputId();item.InputID=&value }
	if event.GetInputIndexPresence()!=nil { value:=event.GetInputIndex();item.InputIndex=&value }
	if event.GetAttemptIdPresence()!=nil { value:=event.GetAttemptId();item.AttemptID=&value }
	if status:=event.GetStatus(); status!=nil { item.Status=publicCallStatus(status.Status) }
	if log:=event.GetLog(); log!=nil { item.Log=append([]byte(nil),log.Data...) }
	if failure:=event.GetError(); failure!=nil { item.Error=callError(failure).(*RemoteCallError) }
	if result:=event.GetResult(); result!=nil {
		inputID:=result.GetInputId();item.InputID=&inputID
		inputIndex:=result.GetInputIndex();item.InputIndex=&inputIndex
		if result.GetYieldIndexPresence()!=nil { yieldIndex:=result.GetYieldIndex();item.YieldIndex=&yieldIndex }
	}
...
func (function *Function[T]) SpawnMap(ctx context.Context, inputs []any)(*BatchCall[T],error){ call,err:=function.spawn(ctx,pb.CallType_CALL_TYPE_BATCH,inputs,true);if err!=nil{return nil,err};return &BatchCall[T]{FunctionCall:call,count:uint64(len(inputs))},nil }
...
func (function *Function[T]) Map(ctx context.Context, inputs <-chan any) (*BatchCall[T], error) {
	...
	open := func(hint string) (grpc.BidiStreamingClient[pb.StreamCallInputsRequest, pb.StreamCallInputsResponse], string, uint64, error) {
		...
		if err != nil { return nil, "", 0, err }
		if err := stream.Send(&pb.StreamCallInputsRequest{Frame: &pb.StreamCallInputsRequest_Call{Call: &pb.CallRef{CallId: call.id}}}); err != nil { return nil, "", 0, err }
		ack, err := stream.Recv()
		if err != nil { return nil, "", 0, err }
		return stream, endpoint, ack.CommittedInputCount, nil
	}
	stream, endpoint, count, err := open(call.endpoint)
	if err != nil { return nil, err }
	for {
		select {
		case <-ctx.Done():
		...
		case input, ok := <-inputs:
			...
			for {
				...
				if sendErr == nil { ack, sendErr = stream.Recv() }
				if sendErr == nil {
					if ack.CommittedInputCount != count+1 { return nil, fmt.Errorf("vmon: committed input count %d, expected %d", ack.CommittedInputCount, count+1) }
				...
				if err != nil { return nil, err }
				if committed > count+1 { return nil, fmt.Errorf("vmon: committed input frontier advanced from %d to %d", count, committed) }
				if committed == count+1 { count = committed; break }
				if committed != count { return nil, fmt.Errorf("vmon: committed input frontier regressed from %d to %d", count, committed) }
			}
		}
	}
}
...
type IndexedResult[T any] struct { InputID string; InputIndex uint64; Value T }
// CompletionResults streams committed results in durable event completion order.
func (batch *BatchCall[T]) CompletionResults(ctx context.Context) (<-chan IndexedResult[T], <-chan error) {
	...
	failures := make(chan error, 1)
	go func() {
		defer close(results); defer close(failures)
		events, eventErrors := batch.Watch(ctx, 0, true)
		for event := range events {
			if event.Kind == CallEventResult && event.InputIndex != nil {
				value, err := batch.Result(ctx, *event.InputIndex)
				if err != nil { failures <- err; return }
				item := IndexedResult[T]{InputIndex:*event.InputIndex, Value:value}
				if event.InputID != nil { item.InputID=*event.InputID }
				select { case results<-item: case <-ctx.Done(): failures<-ctx.Err(); return }
			}
			if event.Status == CallStatusFailed { if event.Error!=nil {failures<-event.Error}else{failures<-errors.New("vmon: batch failed")};return }
			if event.Status == CallStatusCancelled { failures<-ErrCallCancelled;return }
		}
		if err:=<-eventErrors;err!=nil{failures<-err}
	}()
...
type App struct{ Namespace,Name,RevisionID string; client *Client; endpoint string; bindings map[string]*pb.RevisionRef }
...
func lookupApp(ctx context.Context, client *Client, namespace, name string, selector *pb.AppSelector) (*App, error) {
	...
	endpoint, err := client.unary(ctx, "", "get app", func(ctx context.Context,conn grpc.ClientConnInterface,opts ...grpc.CallOption)error{var err error;revision,err=pb.NewFunctionServiceClient(conn).GetApp(ctx,&pb.GetAppRequest{App:selector},opts...);return err})
	if err != nil{return nil,err}
	ref:=revision.GetRef();if ref==nil{return nil,errors.New("vmon: app lookup returned no revision")}
	...
	for _, binding := range revision.Functions { if binding != nil && binding.Revision != nil { app.bindings[binding.Name] = binding.Revision } }
	return app,nil
}
...
func AppFunction[T any](app *App, name string) (*Function[T], error) {
	if app == nil || app.client == nil { return nil, errors.New("vmon: invalid app handle") }
	revision := app.bindings[name]
	if revision == nil { return nil, fmt.Errorf("vmon: app function binding %q not found", name) }
