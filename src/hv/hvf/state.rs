...
//! registers, PSCI power state, and virtual-timer controls) into the [`VcpuState::Hvf`] variant.
//! The KVM backend uses a different variant, so a cross-backend restore fails
//! the `match` below.

use applevisor::prelude::{GicIccReg, Reg, SimdFpReg};

use crate::arch::state::VcpuState;
use crate::hv::SysReg;
use crate::hv::hvf::vcpu::{Vcpu, sys_reg_to_hvf};
