...
//! drains it.  This preserves pollability without letting wakeup storms fill the
//! finite macOS pipe buffer.
    ...
    while !buf.is_empty() {
        ...
        if ret == 0 {
            return Err(io::Error::new(
                ...
                "eventfd pipe write returned zero",
            ));
        }
