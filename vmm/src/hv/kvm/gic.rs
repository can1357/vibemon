...
impl Gic {
	...
	pub fn new(vm: &VmFd, num_cpus: u64) -> Result<Self> {
		let mut create =
		...
		Ok(Self { device, redist_size })
	}

	/// Return `FDT` `reg` pairs: distributor then redistributor.
	pub const fn fdt_reg(&self) -> [u64; 4] {
	...
	#[allow(
		clippy::unused_self,
		reason = "FDT property helpers are exposed consistently as methods"
	)]
	...
	pub const fn device(&self) -> &DeviceFd {
		&self.device
	}
