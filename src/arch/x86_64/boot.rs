...
use linux_loader::loader::{Cmdline, KernelLoader, bzimage::BzImage, load_cmdline};
...
pub fn map_uefi_firmware(
    ...
    if !(buf.len() as u64).is_multiple_of(GUEST_PAGE_SIZE) {
        bail!(
            "firmware {path:?} size {} is not {GUEST_PAGE_SIZE}-byte aligned",
            buf.len()
        );
    }
    if buf.len() as u64 > crate::layout::MEM_32BIT_GAP_SIZE {
        bail!(
            "firmware {path:?} size {} exceeds the x86 MMIO gap",
            buf.len()
        );
    }
...
/// page-aligned as high as possible within `min(lowmem_end, initrd_addr_max+1)`.
pub fn load_initrd(
    path: &Path,
    mem: &GuestMemoryMmap,
    initrd_addr_max: u64,
...
pub fn configure_system(
    ...
    let mut params = boot_params {
        hdr: setup_header.unwrap_or_default(),
        ..Default::default()
    };
