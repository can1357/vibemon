...
//!
//! [`Exit`] is the unified vCPU run result: both backends self-complete MMIO/PIO
//! accesses, so after the run loop fills (read) or consumes (write) `data`, the
//! next [`crate::hv::Vcpu::run`] applies completion (KVM lets the kernel finish
//! the access; HVF writes the destination GPR and advances PC).
...
/// benign wake (EINTR / handled-in-place) that should re-run without recording a
/// VM-exit metric; `Other` is any modeled-but-uninteresting exit.
///
