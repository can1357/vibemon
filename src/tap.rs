...
mod platform {
    ...
    use crate::tap::{TUN_F_CSUM, TUN_F_TSO4, TUN_F_TSO6, VNET_HDR_SIZE};
...
mod platform {
    ...
    #[expect(
        ...
        reason = "field names mirror vmnet's C vmpktdesc layout"
    )]
    struct Vmpktdesc {
    ...
    impl Tap {
        ...
        pub fn open(name: &str, mac: [u8; 6]) -> Result<Self> {
            ...
            if status.status != VMNET_SUCCESS {
                ...
                return Err(vmnet_error(
                    "starting vmnet shared interface",
                    status.status,
                    Some(name),
                ));
            }
        ...
        pub fn set_offloads(offloads: u32) -> Result<()> {
            if offloads == 0 {
                ...
                Err(err(format!(
                    "vmnet does not support negotiated TAP offloads {offloads:#x}"
                )))
            }
        }
    ...
    fn vmnet_io_status(status: VmnetReturn) -> io::Result<()> {
        match status {
            VMNET_SUCCESS => Ok(()),
            VMNET_PACKET_TOO_BIG => Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                status_name(status),
            )),
